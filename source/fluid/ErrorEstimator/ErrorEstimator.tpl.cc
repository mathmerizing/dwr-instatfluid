 ...
 * @date 2018-03-13, new development ErrorEstimator class for heat (begin), UK,
 * MPB
...
ErrorEstimateOnCell<dim>::ErrorEstimateOnCell(
    ...
    const dealii::Mapping<dim> &mapping, const dealii::Quadrature<dim> &quad,
...
template <int dim>
ErrorEstimateOnCell<dim>::ErrorEstimateOnCell(
    const ErrorEstimateOnCell &scratch)
    : fe_values_high(scratch.fe_values_high.get_mapping(),
                     scratch.fe_values_high.get_fe(),
                     scratch.fe_values_high.get_quadrature(),
                     scratch.fe_values_high.get_update_flags()),
      fe_values_pu(scratch.fe_values_pu.get_mapping(),
      ...
      value_symgrad_z_k_z_kh_convection(
...
template <int dim>
ErrorEstimates<dim>::ErrorEstimates(const dealii::DoFHandler<dim> &dof_high,
                                    const dealii::DoFHandler<dim> &dof_pu,
                                    const dealii::FiniteElement<dim> &fe_high,
                                    const dealii::FiniteElement<dim> &fe_pu,
...
template <int dim>
ErrorEstimateOnCell<dim>::ErrorEstimateOnCell(
    const ErrorEstimateOnCell &copydata)
...
template <int dim>
void ErrorEstimator<dim>::estimate_on_slab(
  ...
  // INFO: replace_linearization_points and replace_weights are being hard coded
  // in the error estimator
  //       They have a negligible impact on the effectivity index and hard
  //       coding them makes the estimator more readable.

  primal.um_on_tn = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();

  pu.constraints = std::make_shared<dealii::AffineConstraints<
      double> >();  // slab->space.pu.fe_info->constraints;
  ...
  // check low.solution_can_be_patchwise_interpolated
  {
    ...
    for (; low_cell != low_endc; ++low_cell) {
      if (!(low_cell->has_children() || low_cell->is_artificial())) {
        ...
        DTM::pout
            << "WARNING: your tria on this slab: " << std::endl
            << "         ( " << slab->t_m << " , " << slab->t_n << " )"
            << std::endl
            << "         is not at least once globally refined." << std::endl
            << "         Patchwise higher order interpolation in space is not"
            << std::endl
            << "         possible, keeping the order instead." << std::endl
            << std::endl;
  ...
  std::vector<dealii::types::global_dof_index> low_local_dof_indices_time(
  ...
  dealii::QGauss<dim> quad_cell(
      std::max(std::max(slab->space.high.fe_info->fe->base_element(0)
                            .base_element(0)
                            .tensor_degree(),
                        slab->space.high.fe_info->fe->base_element(0)
                            .base_element(1)
                            .tensor_degree()),
  ...
  auto tmp_high_lod = std::make_shared<dealii::IndexSet>(
      idealii::SlabDoFTools::extract_locally_owned_dofs(
          slab->space.high.fe_info->dof, slab->time.high.fe_info->dof));

  auto tmp_high_lrd = std::make_shared<dealii::IndexSet>(
  ...
  // GETTING Z_k^{ρ_k}
  //
  auto high_z_k_rho_k_slab =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  high_z_k_rho_k_slab->reinit(*tmp_high_lod, *tmp_high_lrd, mpi_comm);

  if (!dual_order.compare("high") ||
      !dual_order.compare("high-time"))  // dual = { [time = high, space =
                                         // high], [time = high, space = low] }
  {
    // computation of z_kh^(1,2) from z_kh^(2,2)
    get_back_interpolated_time_slab_w(  // for mixed order and semi-mixed order
        slab, slab->space.high.fe_info->dof, high_z_slab, high_z_k_rho_k_slab);
  } else
    AssertThrow(false, dealii::ExcNotImplemented());
  ...
  auto high_z_kh_slab =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  high_z_kh_slab->reinit(*tmp_high_lod, *tmp_high_lrd, mpi_comm);
                                         ...
                                         // high], [time = high, space = low] }
  {
    ...
    get_back_interpolated_space_slab(  // for mixed order and semi-mixed order
        slab, high_z_k_rho_h_slab, high_z_kh_slab);
  } else
    AssertThrow(false, dealii::ExcNotImplemented());
  ...
  // GETTING U_k == U_kh
  //
  auto tmp_lod = std::make_shared<dealii::IndexSet>(
      idealii::SlabDoFTools::extract_locally_owned_dofs(
          slab->space.low.fe_info->dof, slab->time.high.fe_info->dof));

  auto tmp_lrd = std::make_shared<dealii::IndexSet>(
      idealii::SlabDoFTools::extract_locally_relevant_dofs(
          slab->space.low.fe_info->dof, slab->time.high.fe_info->dof));

  // interpolate u_kh^(1,1) in time
  auto low_interpolated_time_u_slab =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  low_interpolated_time_u_slab->reinit(*tmp_lod, *tmp_lrd, mpi_comm);

  get_interpolated_time_slab_w(slab, slab->space.low.fe_info->dof, u->x[0],
  ...
  high_u_slab->reinit(*tmp_high_lod, *tmp_high_lrd, mpi_comm);

  get_interpolated_space_slab(slab, low_interpolated_time_u_slab, high_u_slab);
  ...
  auto primal_um_on_tm =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  primal_um_on_tm->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
                          *slab->space.primal.fe_info->locally_relevant_dofs,
                          mpi_comm);

  // reinit vectors to high space
  for (auto vec :
       {&high_u_kh_m_on_tm, &high_u_k_m_on_tm, &high_u_k_p_on_tm,
        &high_u_kh_p_on_tm, &high_z_kh_p_on_tm, &high_z_k_rho_k_p_on_tm,
        &high_z_k_rho_h_p_on_tm, &high_z_p_on_tm, &high_z_on_tq,
        &high_z_kh_on_tq, &high_z_k_rho_k_on_tq, &high_z_k_rho_h_on_tq,
        &high_u_kh_on_tq, &high_dt_u_kh_on_tq}) {
    *vec = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
    (*vec)->reinit(*slab->space.high.fe_info->locally_owned_dofs,
                   *slab->space.high.fe_info->locally_relevant_dofs, mpi_comm);
  }

  auto high_owned = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  high_owned->reinit(*slab->space.high.fe_info->locally_owned_dofs, mpi_comm);

  if (slab == grid->slabs.begin()) {
    // slab n == 1: interpolate initial value function u_0 to high solution
    // space
    Assert(primal_um_on_tm.use_count(), dealii::ExcNotInitialized());
    Assert(primal_um_on_tm->size(), dealii::ExcNotInitialized());
    tmp_owned->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
                      mpi_comm);
    *tmp_owned = 0.;
    // our case: u_0 = 0
    ...
    // (Note: after the first dwr-loop the initial grid could have hanging
    // nodes)
    ...
    *primal_um_on_tm = *tmp_owned;
    ...
    if (std::prev(slab)->space.primal.fe_info->dof->n_dofs() !=
    ...
    // primal_um_on_tm has actually already been stored with a divergence free
    // projection in um
  ...
  Assert(!primal_order.compare("low"), dealii::ExcNotImplemented());
  {
    ...
    dealii::FETools::interpolate(
        // primal/low solution
        *slab->space.primal.fe_info->dof, *primal_um_on_tm,
        // high solution
        *slab->space.high.fe_info->dof,
        *slab->space.high.fe_info->hanging_node_constraints, *high_owned);
  ...
  for (; cell_time != endc_time; ++cell_time, ++low_cell_time) {
    ...
    Assert(cell_time->center()[0] == low_cell_time->center()[0],
           dealii::ExcInternalError());

    Assert(low_local_dof_indices_time.size() ==
               slab->time.low.fe_info->fe->dofs_per_cell,
           dealii::ExcNotInitialized());

    low_cell_time->get_dof_indices(low_local_dof_indices_time);

    // face values: [t_m, t_n]
    dealii::FEValues<1> fe_face_values_time(
        *slab->time.high.fe_info->mapping, *slab->time.high.fe_info->fe,
        dealii::QGaussLobatto<1>(2), dealii::update_quadrature_points);
    ...
    //
    get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
            slab->space.high.fe_info->dof, cell_time, high_u_slab,
            fe_face_values_time.quadrature_point(0)[0], high_u_kh_p_on_tm);
    ...
    //
    get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
            slab->space.high.fe_info->dof, cell_time, high_z_slab,
            fe_face_values_time.quadrature_point(0)[0], high_z_p_on_tm);
    get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
            slab->space.high.fe_info->dof, cell_time, high_z_k_rho_k_slab,
            fe_face_values_time.quadrature_point(0)[0], high_z_k_rho_k_p_on_tm);
    get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
            slab->space.high.fe_info->dof, cell_time, high_z_k_rho_h_slab,
            fe_face_values_time.quadrature_point(0)[0], high_z_k_rho_h_p_on_tm);
    get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
            slab->space.high.fe_info->dof, cell_time, high_z_kh_slab,
            fe_face_values_time.quadrature_point(0)[0], high_z_kh_p_on_tm);
    ...
    dealii::WorkStream::run(
        ...
        std::bind(&ErrorEstimator<dim>::assemble_local_error_tm, this,
                  std::placeholders::_1, std::placeholders::_2,
                  std::placeholders::_3),
        std::bind(&ErrorEstimator<dim>::copy_local_error, this,
                  std::placeholders::_1),
        Assembly::Scratch::ErrorEstimates<dim>(
            *slab->space.high.fe_info->dof, *slab->space.pu.fe_info->dof,
            *slab->space.high.fe_info->fe, *slab->space.pu.fe_info->fe,
            *slab->space.high.fe_info->mapping, quad_cell,
            //
    ...
    dealii::FEValues<1> fe_values_time(
        *slab->time.high.fe_info->mapping, *slab->time.high.fe_info->fe,
        dealii::QGauss<1>(slab->time.high.fe_info->fe->tensor_degree() + 5),
    ...
    for (unsigned int qt{0}; qt < fe_values_time.n_quadrature_points; ++qt) {
      ...
      get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
              slab->space.high.fe_info->dof, cell_time, high_u_slab,
              fe_values_time.quadrature_point(qt)[0], high_u_kh_on_tq);

      get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
              slab->space.high.fe_info->dof, cell_time, high_z_slab,
              fe_values_time.quadrature_point(qt)[0], high_z_on_tq);

      get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
              slab->space.high.fe_info->dof, cell_time, high_z_k_rho_k_slab,
              fe_values_time.quadrature_point(qt)[0], high_z_k_rho_k_on_tq);

      get_dt_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
                 slab->space.high.fe_info->dof, cell_time, high_u_slab,
                 fe_values_time.quadrature_point(qt)[0], high_dt_u_kh_on_tq);

      get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
              slab->space.high.fe_info->dof, cell_time, high_z_k_rho_h_slab,
              fe_values_time.quadrature_point(qt)[0], high_z_k_rho_h_on_tq);

      get_w_t(slab->time.high.fe_info->fe, slab->time.high.fe_info->mapping,
              slab->space.high.fe_info->dof, cell_time, high_z_kh_slab,
              fe_values_time.quadrature_point(qt)[0], high_z_kh_on_tq);
      ...
      dealii::WorkStream::run(
          ...
          CellFilter(dealii::IteratorFilters::LocallyOwnedCell(),
                     slab->space.high.fe_info->dof->end()),
          std::bind(&ErrorEstimator<dim>::assemble_local_error, this,
                    std::placeholders::_1, std::placeholders::_2,
                    std::placeholders::_3),
          std::bind(&ErrorEstimator<dim>::copy_local_error, this,
                    std::placeholders::_1),
...
template <int dim>
void ErrorEstimator<dim>::get_w_t(
    ...
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> w, const double &t,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> &w_t) {
  Assert(w.use_count(), dealii::ExcNotInitialized());

  dealii::FEValues<1> fe_face_values_time(*time_mapping, *time_fe,
...
#ifdef DEBUG
  {
    ...
    Assert((t >= fe_face_values_time.quadrature_point(0)[0]),
           dealii::ExcInvalidState());

    // check if <= t_n of dual_cell_time
    Assert((t <= fe_face_values_time.quadrature_point(1)[0]),
           dealii::ExcInvalidState());
  }
#endif
  ...
  output_time_points[0][0] =
      (t - fe_face_values_time.quadrature_point(0)[0]) / cell_time->diameter();

  // eval
  dealii::FEValues<1> fe_values_time(*time_mapping, *time_fe,
  ...
  *tmpvec = 0.;
  Assert(fe_values_time.n_quadrature_points == 1, dealii::ExcInternalError());

  // only a single QP, so no loop needed
  unsigned int qt{0};

  // evaluate solution for t_q
  for (unsigned int jj{0}; jj < time_fe->dofs_per_cell; ++jj) {
    dealii::IndexSet::ElementIterator lri = loe.begin();
    dealii::IndexSet::ElementIterator lre = loe.end();
    for (; lri != lre; lri++) {
      double tmp = (*w)[*lri
                        // time offset
                        + space_dof->n_dofs() * local_dof_indices[jj]] *
...
template <int dim>
void ErrorEstimator<dim>::get_dt_w_t(
    std::shared_ptr<dealii::FiniteElement<1> > time_fe,
    std::shared_ptr<dealii::Mapping<1> > time_mapping,
    std::shared_ptr<dealii::DoFHandler<dim> > space_dof,
    const typename dealii::DoFHandler<1>::active_cell_iterator &cell_time,
...
template <int dim>
void ErrorEstimator<dim>::interpolate_space(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector>
        &interpolated_space_w) {
  Assert(w.use_count(), dealii::ExcNotInitialized());
  Assert((w->size() == slab->space.low.fe_info->dof->n_dofs()),
         dealii::ExcMessage(
  ...
  tmp->reinit(interpolated_space_w->locally_owned_elements(), mpi_comm);
  ...
  dealii::FETools::interpolate(
      // low solution
      *slab->space.low.fe_info->dof, *w,
      // high solution
      *slab->space.high.fe_info->dof,
      *slab->space.high.fe_info->hanging_node_constraints, *tmp);
  *interpolated_space_w = *tmp;
}

template <int dim>
void ErrorEstimator<dim>::patchwise_high_order_interpolate_space(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector>
  ...
  // Therefore, the spatial triangulation must be at least once globally
  // refined.
  //
  ...
  auto tmpvec = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  tmpvec->reinit(higher_order_space_w->locally_owned_elements(), mpi_comm);
  ...
  if (low.solution_can_be_patchwise_interpolated) {
    ...
    dealii::FETools::extrapolate(
        *slab->space.low.fe_info->dof, *w,
        // high solution
        *slab->space.high.fe_info->dof,
        *slab->space.high.fe_info->hanging_node_constraints, *tmpvec);
  } else {
    // interpolate low dof vector to high dof vector
    dealii::FETools::interpolate(
        // low solution
...
template <int dim>
void ErrorEstimator<dim>::back_interpolate_space(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector>
  ...
  Assert(w.use_count(), dealii::ExcNotInitialized());
  Assert((w->size() == slab->space.high.fe_info->dof->n_dofs()),
         dealii::ExcMessage(
             "Internal dimensions error: "
  ...
  auto tmp = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  tmp->reinit(back_interpolated_space_w->locally_owned_elements(), mpi_comm);
...
template <int dim>
void ErrorEstimator<dim>::get_patchwise_high_order_interpolated_space_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> slab_w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> &high_slab_w) {
  // TODO: adapt this for multiple cells per slab, as in
  // get_interpolated_time_slab_w

  auto tmp_high_slab =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  tmp_high_slab->reinit(high_slab_w->locally_owned_elements(), mpi_comm);

  // slab_w evaluated at temporal quadrature point
  auto slab_w_tq = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  slab_w_tq->reinit(*slab->space.low.fe_info->locally_owned_dofs,
                    *slab->space.low.fe_info->locally_relevant_dofs, mpi_comm);
  *slab_w_tq = 0.;

  auto slab_w_tq_tmp =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  slab_w_tq_tmp->reinit(*slab->space.low.fe_info->locally_owned_dofs, mpi_comm);

  // higher order reconstruction in space of slab_w evaluated at temporal
  // quadrature point
  auto high_slab_w_tq =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  high_slab_w_tq->reinit(*slab->space.high.fe_info->locally_owned_dofs,
  ...
  for (unsigned int ii{0}; ii < slab->time.high.fe_info->dof->n_dofs(); ++ii) {
    ...
    for (auto i : *slab->space.low.fe_info->locally_owned_dofs)
      (*slab_w_tq_tmp)[i] =
          (*slab_w)[i + slab->space.low.fe_info->dof->n_dofs() * ii];
    *slab_w_tq = *slab_w_tq_tmp;

    // use higher order interpolation in space to go from slab_w_tq to
    // high_slab_w_tq
    patchwise_high_order_interpolate_space(slab, slab_w_tq, high_slab_w_tq);

    // write high_slab_w_tq into high_slab_w
    for (auto i : *slab->space.high.fe_info->locally_owned_dofs)
      (*tmp_high_slab)[i + slab->space.high.fe_info->dof->n_dofs() * ii] =
          (*high_slab_w_tq)[i];
  }
...
template <int dim>
void ErrorEstimator<dim>::get_interpolated_space_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> slab_w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> &high_slab_w) {
  // TODO: adapt this for multiple cells per slab, as in
  ...
  for (unsigned int ii{0}; ii < slab->time.high.fe_info->dof->n_dofs(); ++ii) {
    ...
    *slab_w_tq = *slab_w_tq_tmp;
    ...
    interpolate_space(slab, slab_w_tq, high_slab_w_tq);
...
template <int dim>
void ErrorEstimator<dim>::get_back_interpolated_space_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> slab_w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> &high_slab_w) {
  // TODO: adapt this for multiple cells per slab, as in
  ...
  // slab_w evaluated at temporal quadrature point
  auto slab_w_tq = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  slab_w_tq->reinit(*slab->space.high.fe_info->locally_owned_dofs,
                    *slab->space.high.fe_info->locally_relevant_dofs, mpi_comm);
  *slab_w_tq = 0.;

  auto slab_w_tq_tmp =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  slab_w_tq_tmp->reinit(*slab->space.high.fe_info->locally_owned_dofs,
                        mpi_comm);
  ...
  for (unsigned int ii{0}; ii < slab->time.high.fe_info->dof->n_dofs(); ++ii) {
    ...
    for (auto i : *slab->space.high.fe_info->locally_owned_dofs)
      (*slab_w_tq_tmp)[i] =
          (*slab_w)[i + slab->space.high.fe_info->dof->n_dofs() * ii];
    *slab_w_tq = *slab_w_tq_tmp;

    // use back interpolation in space to go from slab_w_tq to high_slab_w_tq
    back_interpolate_space(slab, slab_w_tq, high_slab_w_tq);
...
template <int dim>
void ErrorEstimator<dim>::get_high_back_interpolated_time_slab_w(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> slab_w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector>
  ...
  high_slab_w->reinit(high_owned_dofs, high_relevant_dofs, mpi_comm);
  *high_slab_w = 0.;
  ...
  *high_slab_w_tq = 0.;
  ...
  for (unsigned int ii{0}; ii < slab->time.high.fe_info->dof->n_dofs(); ++ii) {
    ...
    *slab_w_tq = *slab_w_tq_tmp;
    ...
    // write high_slab_w_tq into high_slab_w
    for (auto i : *slab->space.high.fe_info->locally_owned_dofs)
      (*high_slab_w)[i + slab->space.high.fe_info->dof->n_dofs() * ii] =
          (*high_slab_w_tq)[i];
  }

  get_back_interpolated_time_slab_w(slab, slab->space.high.fe_info->dof,
                                    high_slab_w, back_interpolated_time_slab_w);
}

template <int dim>
void ErrorEstimator<dim>::get_back_interpolated_time_slab_w(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::DoFHandler<dim> > space_dof,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> slab_w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector>
  ...
  Assert(space_dof.use_count(), dealii::ExcNotInitialized());
  ...
  Assert(slab_w.use_count(), dealii::ExcNotInitialized());

  Assert(slab_w->size(), dealii::ExcNotInitialized());

  Assert(back_interpolated_time_slab_w.use_count(),
         dealii::ExcNotInitialized());

  dealii::IndexSet low_restricted_owned_dofs =
      idealii::SlabDoFTools::extract_locally_owned_dofs(
          space_dof, slab->time.low.fe_info->dof);
  ...
  dealii::IndexSet high_restricted_owned_dofs =
      idealii::SlabDoFTools::extract_locally_owned_dofs(
          space_dof, slab->time.high.fe_info->dof);
  ...
  auto low_restricted_time_slab_w =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  low_restricted_time_slab_w->reinit(low_restricted_owned_dofs,
                                     low_restricted_relevant_dofs, mpi_comm);
  *low_restricted_time_slab_w = 0.;

  auto low_restricted_tmp =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  low_restricted_tmp->reinit(low_restricted_owned_dofs, mpi_comm);

  // init result vector
  //	back_interpolated_time_slab_w = std::make_shared<
  //dealii::TrilinosWrappers::MPI::Vector > ();
  back_interpolated_time_slab_w->reinit(
      high_restricted_owned_dofs, high_restricted_relevant_dofs, mpi_comm);
  *back_interpolated_time_slab_w = 0.;

  auto back_tmp = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  back_tmp->reinit(high_restricted_owned_dofs, mpi_comm);

  // prepare local dof mappings
  std::vector<dealii::types::global_dof_index> low_local_dof_indices_time(
      slab->time.low.fe_info->fe->dofs_per_cell);

  std::vector<dealii::types::global_dof_index> high_local_dof_indices_time(
  ...
  dealii::FEValues<1> high_fe_values_time(
      *slab->time.high.fe_info->mapping, *slab->time.high.fe_info->fe,
      dealii::Quadrature<1>(
          slab->time.low.fe_info->fe->get_unit_support_points()),
      dealii::update_values);

  dealii::FEValues<1> low_fe_values_time(
      *slab->time.low.fe_info->mapping, *slab->time.low.fe_info->fe,
      dealii::Quadrature<1>(
          slab->time.high.fe_info->fe->get_unit_support_points()),
      dealii::update_values);
  ...
  for (; cell_time != endc_time; ++cell_time, ++low_cell_time) {
    ...
    Assert(low_cell_time != slab->time.low.fe_info->dof->end(),
           dealii::ExcInternalError());
    ...
    Assert(high_local_dof_indices_time.size() ==
               slab->time.high.fe_info->fe->dofs_per_cell,
           dealii::ExcNotInitialized());

    cell_time->get_dof_indices(high_local_dof_indices_time);
    ...
    Assert(high_fe_values_time.n_quadrature_points ==
    ...
    Assert(low_fe_values_time.n_quadrature_points ==
    ...
    for (unsigned int qt{0}; qt < high_fe_values_time.n_quadrature_points;
         ++qt) {
      for (unsigned int jj{0}; jj < high_fe_values_time.get_fe().dofs_per_cell;
           ++jj) {
        dealii::IndexSet::ElementIterator lri =
            space_dof->locally_owned_dofs().begin();
        dealii::IndexSet::ElementIterator lre =
            space_dof->locally_owned_dofs().end();
        for (; lri != lre; lri++) {
          (*low_restricted_tmp)[*lri
                                // time offset
                                + space_dof->n_dofs() *
              ...
              (*slab_w)[*lri
                        // time offset
                        +
                        space_dof->n_dofs() * high_local_dof_indices_time[jj]] *
    ...
    *low_restricted_time_slab_w = *low_restricted_tmp;
    ...
    for (unsigned int qt{0}; qt < low_fe_values_time.n_quadrature_points;
         ++qt) {
      for (unsigned int jj{0}; jj < low_fe_values_time.get_fe().dofs_per_cell;
           ++jj) {
        dealii::IndexSet::ElementIterator lri =
            space_dof->locally_owned_dofs().begin();
        dealii::IndexSet::ElementIterator lre =
        ...
        for (; lri != lre; lri++) {
          (*back_tmp)[*lri
                      // time offset
                      +
                      space_dof->n_dofs() * high_local_dof_indices_time[qt]] +=
              (*low_restricted_time_slab_w)
                  [*lri
                   // time offset
                   + space_dof->n_dofs() * low_local_dof_indices_time[jj]] *
...
template <int dim>
void ErrorEstimator<dim>::get_interpolated_time_slab_w(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::DoFHandler<dim> > space_dof,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> slab_w,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector>
        &interpolated_time_slab_w) {
  // TODO: does this also work for multiple cells per slab?!
  ...
  interpolated_time_slab_w =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  interpolated_time_slab_w->reinit(high_restricted_owned_dofs,
                                   high_restricted_relevant_dofs, mpi_comm);
  *interpolated_time_slab_w = 0.;

  auto interpolated_time_slab_w_tmp =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  interpolated_time_slab_w_tmp->reinit(high_restricted_owned_dofs, mpi_comm);
      ...
      *slab->time.low.fe_info->mapping, *slab->time.low.fe_info->fe,
      dealii::Quadrature<1>(
          slab->time.high.fe_info->fe
              ->get_unit_support_points()),  //*high_quad_time,
      dealii::update_values);
...
template <int dim>
void ErrorEstimator<dim>::assemble_local_error_tm(
...
template <int dim>
void ErrorEstimator<dim>::assemble_error_on_cell_tm(
    const typename dealii::DoFHandler<dim>::active_cell_iterator &cell,
    Assembly::Scratch::ErrorEstimateOnCell<dim> &scratch,
    Assembly::CopyData::ErrorEstimateOnCell<dim> &copydata) {
  typename dealii::DoFHandler<dim>::active_cell_iterator cell_high(
      &cell->get_triangulation(), cell->level(), cell->index(),
      &scratch.dof_high);

  typename dealii::DoFHandler<dim>::active_cell_iterator cell_pu(
      &cell->get_triangulation(), cell->level(), cell->index(),
      &scratch.dof_pu);
  ...
  for (scratch.j = 0; scratch.j < scratch.fe_values_high.get_fe().dofs_per_cell;
    ...
    // BUT: if we replace the linearization point, then here u_k is being
    // replaced by u_kh --> this has already been done in the code before! 2a)
    // get u_k(t_m^-)
  ...
  // For the jump we always only need the convection components of all the
  // vectors
  ...
  std::vector<dealii::types::global_dof_index> tmp_local_dof_indices_pu(
      scratch.fe_values_pu.get_fe().dofs_per_cell);
  ...
  for (unsigned int i{0}; i < scratch.fe_values_pu.get_fe().dofs_per_cell; ++i)
    copydata.local_dof_indices_pu[i] =
        tmp_local_dof_indices_pu[i] + cell_time_index * scratch.dof_pu.n_dofs();

  // assemble PU
  for (scratch.q = 0; scratch.q < scratch.fe_values_pu.n_quadrature_points;
       ++scratch.q) {
    scratch.JxW = scratch.fe_values_pu.JxW(scratch.q);

    // loop over all basis functions to get the shape values
    for (scratch.j = 0;
         scratch.j < scratch.fe_values_high.get_fe().dofs_per_cell;
         ++scratch.j) {
    ...
    for (scratch.j = 0;
      ...
      scratch.value_z_z_k_convection += (scratch.local_z_p[scratch.j] -
                                         scratch.local_z_k_rho_k_p[scratch.j]) *
                                        scratch.phi_convection[scratch.j];
    ...
    for (scratch.j = 0; scratch.j < scratch.fe_values_pu.get_fe().dofs_per_cell;
      ...
      copydata.local_eta_k_vector[scratch.j] -=
          (
              ...
              scratch.value_jump_u_k_convection *
              scratch.value_z_z_k_convection * scratch.chi[scratch.j]) *
          scratch.JxW;
      ...
      copydata.local_eta_h_vector[scratch.j] -=
          (
              ...
              scratch.value_jump_u_kh_convection *
              scratch.value_z_k_z_kh_convection * scratch.chi[scratch.j]) *
          scratch.JxW;
    }
  }  // for q
}
...
template <int dim>
void ErrorEstimator<dim>::assemble_local_error(
    const typename dealii::DoFHandler<dim>::active_cell_iterator &cell,
    Assembly::Scratch::ErrorEstimates<dim> &scratch,
    Assembly::CopyData::ErrorEstimates<dim> &copydata) {
  ////////////////////////////////////////////////////////////////////////
...
template <int dim>
void ErrorEstimator<dim>::assemble_error_on_cell(
    const typename dealii::DoFHandler<dim>::active_cell_iterator &cell,
    Assembly::Scratch::ErrorEstimateOnCell<dim> &scratch,
    Assembly::CopyData::ErrorEstimateOnCell<dim> &copydata) {
  typename dealii::DoFHandler<dim>::active_cell_iterator cell_high(
  ...
  for (scratch.q = 0; scratch.q < scratch.fe_values_pu.n_quadrature_points;
    ...
    for (scratch.j = 0;
      ...
      scratch.symgrad_phi_convection[scratch.j] =
          scratch.fe_values_high[convection].symmetric_gradient(scratch.j,
                                                                scratch.q);
    ...
    for (scratch.j = 0;
      ...
      scratch.value_dt_u_k_convection +=
          scratch.local_dtu_k_tq[scratch.j] * scratch.phi_convection[scratch.j];
      ...
      // (∂_t v_kh, [z^v_k-z^v_kh]χ_i)
      scratch.value_dt_u_kh_convection += scratch.local_dtu_kh_tq[scratch.j] *
                                          scratch.phi_convection[scratch.j];
      scratch.value_z_k_z_kh_convection +=
          (scratch.local_z_k_rho_h_tq[scratch.j] -
           scratch.local_z_kh_tq[scratch.j]) *
      ...
      scratch.value_u_k_pressure +=
          scratch.local_u_k_tq[scratch.j] * scratch.phi_pressure[scratch.j];
      ...
      scratch.value_u_kh_pressure +=
          scratch.local_u_kh_tq[scratch.j] * scratch.phi_pressure[scratch.j];
      ...
      if (nonlinear) {
        ...
        scratch.value_u_k_convection +=
            scratch.local_u_k_tq[scratch.j] * scratch.phi_convection[scratch.j];
        scratch.value_u_kh_convection += scratch.local_u_kh_tq[scratch.j] *
                                         scratch.phi_convection[scratch.j];
      }
      ...
      // (∇ ·v_k, [z^p-z^p_k]χ_i)
      scratch.value_div_u_k_convection += scratch.local_u_k_tq[scratch.j] *
                                          scratch.div_phi_convection[scratch.j];
      scratch.value_z_z_k_pressure += (scratch.local_z_tq[scratch.j] -
                                       scratch.local_z_k_rho_k_tq[scratch.j]) *
                                      scratch.phi_pressure[scratch.j];
    ...
    for (scratch.j = 0; scratch.j < scratch.fe_values_pu.get_fe().dofs_per_cell;
      ...
      copydata.local_eta_k_vector[scratch.j] -=
           ...
           * scratch.value_dt_u_k_convection * scratch.value_z_z_k_convection *
           scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);
      ...
      if (symmetric_stress) {
        ...
        copydata.local_eta_k_vector[scratch.j] -=
            (2. * scratch.value_viscosity *
             scalar_product(scratch.value_symgrad_u_k_convection,
                            scratch.value_symgrad_z_z_k_convection) *
             scratch.JxW * cell_time_JxW);
        ...
        copydata.local_eta_h_vector[scratch.j] -=
            (2. * scratch.value_viscosity *
             scalar_product(scratch.value_symgrad_u_kh_convection,
                            scratch.value_symgrad_z_k_z_kh_convection) *
             scratch.JxW * cell_time_JxW);
      } else {
        // unsymmetric stress tensor
        ...
        // ν(∇v_k, ∇[(z^v-z^v_k)χ_i])) = ν(∇v_k, ∇[z^v-z^v_k]χ_i) + ν(∇v_k,
        // (z^v-z^v_k)⊗(∇χ_i))
        //  I) ν(∇v_k, ∇[z^v-z^v_k]χ_i)
        copydata.local_eta_k_vector[scratch.j] -=
            (scratch.value_viscosity *
             scalar_product(scratch.value_grad_u_k_convection,
                            scratch.value_grad_z_z_k_convection) *
             scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);
        ...
        // using the product rule:
        // ν(∇v_kh, ∇[(z^v_k-z^v_kh)χ_i])) = ν(∇v_kh, ∇[z^v_k-z^v_kh]χ_i) +
        // ν(∇v_kh, (z^v_k-z^v_kh)⊗(∇χ_i))
        //  I) ν(∇v_kh, ∇[z^v_k-z^v_kh]χ_i)
        copydata.local_eta_h_vector[scratch.j] -=
            (scratch.value_viscosity *
             scalar_product(scratch.value_grad_u_kh_convection,
                            scratch.value_grad_z_k_z_kh_convection) *
             scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);
      ...
      // using the product rule:
      // -(p_k, ∇·[(z^v-z^v_k)χ_i]) = -(p_k, ∇·[z^v-z^v_k]χ_i) -(p_k,
      // (z^v-z^v_k) · (∇χ_i))
      //  I) -(p_k, ∇·[z^v-z^v_k]χ_i)
      copydata.local_eta_k_vector[scratch.j] += (  // plus here !
          scratch.value_u_k_pressure * scratch.value_div_z_z_k_convection *
          scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);

      // II) -(p_k, (z^v-z^v_k) · (∇χ_i))
      copydata.local_eta_k_vector[scratch.j] += (  // plus here !
          scratch.value_u_k_pressure *
          scalar_product(scratch.value_z_z_k_convection,
                         scratch.grad_chi[scratch.j]) *
          scratch.JxW * cell_time_JxW);

      // using the product rule:
      // -(p_kh, ∇·[(z^v_k-z^v_kh)χ_i]) = -(p_kh, ∇·[z^v_k-z^v_kh]χ_i) -(p_kh,
      // (z^v_k-z^v_kh) · (∇χ_i))
      //  I) -(p_kh, ∇·[z^v_k-z^v_kh]χ_i)
      copydata.local_eta_h_vector[scratch.j] += (  // plus here !
          scratch.value_u_kh_pressure * scratch.value_div_z_k_z_kh_convection *
          scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);

      // II) -(p_kh, (z^v_k-z^v_kh) · (∇χ_i))
      copydata.local_eta_h_vector[scratch.j] += (  // plus here !
          scratch.value_u_kh_pressure *
          scalar_product(scratch.value_z_k_z_kh_convection,
                         scratch.grad_chi[scratch.j]) *
          scratch.JxW * cell_time_JxW);
      ...
      // (∇ ·v_k, [z^p-z^p_k]χ_i)
      copydata.local_eta_k_vector[scratch.j] -=
          (scratch.value_div_u_k_convection * scratch.value_z_z_k_pressure *
           scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);

      // (∇ ·v_kh, [z^p_k-z^p_kh]χ_i)
      copydata.local_eta_h_vector[scratch.j] -=
          (scratch.value_div_u_kh_convection * scratch.value_z_k_z_kh_pressure *
           scratch.chi[scratch.j] * scratch.JxW * cell_time_JxW);
...
template <int dim>
void ErrorEstimator<dim>::copy_local_error(
    const Assembly::CopyData::ErrorEstimates<dim> &copydata) {
  pu.constraints->distribute_local_to_global(copydata.cell.local_eta_h_vector,
                                             copydata.cell.local_dof_indices_pu,
                                             *error_estimator.x_h);

  pu.constraints->distribute_local_to_global(copydata.cell.local_eta_k_vector,
                                             copydata.cell.local_dof_indices_pu,
                                             *error_estimator.x_k);
}

template <int dim>
void ErrorEstimator<dim>::init(
    MPI_Comm _mpi_comm, std::shared_ptr<dealii::Function<dim> > _viscosity,
    std::shared_ptr<fluid::Grid<dim> > _grid, bool use_symmetric_stress,
    bool _replace_linearization_point, bool _replace_weights,
    std::string _primal_order, std::string _dual_order, bool _nonlinear) {
  mpi_comm = _mpi_comm;
