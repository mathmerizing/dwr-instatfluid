 ...
 * \times (p_1^3,p_2^3) \} \f$ with one mesh cell; cf. deal.II
 * documentation.</li>
 *
 * <li>subdivided_hyper_rectangle: same as hyper_cube but allows for
 * (an-)isotropic refinement upon construction; cf. deal.II documentation.</li>
 *
 ...
 *
 * <li>Lshape: generates \f$ \Omega_h=\{ (0,1)^d \} \setminus \{ (0.5,1)^d \}
 * \f$, \f$ d=2,3 \f$ with 3 mesh cells.</li>
 * </ul>
 *
 ...
 *
 * <li>channel_without_cylinder_2D: creates the grid for the 2D flow around a
 * cylinder from Schaefer/Turek 1996</li>
 *
 * </ul>
 *
 ...
 */
template <int dim>
void TriaGenerator<dim>::generate(
  ...
  if (TriaGenerator_Type.compare("hyper_cube") == 0) {
    AssertThrow(options.size() == 2,
                dealii::ExcMessage("TriaGenerator Options invalid, "
                                   "please check your input file data."));

    dealii::GridGenerator::hyper_cube(*tria, std::stod(options.at(0)),
      ...
      for (unsigned int i(0); i < dim; ++i) {
        try {
          ...
          throw dealii::ExcMessage(
              "TriaGenerator Options invalid, please check your input file "
              "data.");
  ...
  if (TriaGenerator_Type.compare("Lshape") == 0) {
...

    {
      ...
      if (dim == 2) {
        node[0] = 0.0;
        node[1] = 0.0;
        vertices.push_back(node);  // 0
        node[0] = 0.5;
        node[1] = 0.0;
        vertices.push_back(node);  // 1
        node[0] = 1.0;
        node[1] = 0.0;
        vertices.push_back(node);  // 2

        node[0] = 0.0;
        node[1] = 0.5;
        vertices.push_back(node);  // 3
        node[0] = 0.5;
        node[1] = 0.5;
        vertices.push_back(node);  // 4
        node[0] = 1.0;
        node[1] = 0.5;
        vertices.push_back(node);  // 5

        node[0] = 0.0;
        node[1] = 1.0;
        vertices.push_back(node);  // 6
        node[0] = 0.5;
        node[1] = 1.0;
        vertices.push_back(node);  // 7
      } else if (dim == 3) {
        node[2] = 0.0;
        node[0] = 0.0;
        node[1] = 0.0;
        ...
        node[0] = 0.0;
        node[1] = 0.0;
        vertices.push_back(node);  // 8
        node[0] = 0.5;
        node[1] = 0.0;
        vertices.push_back(node);  // 9
        node[0] = 1.0;
        node[1] = 0.0;
        vertices.push_back(node);  // 10

        node[0] = 0.0;
        node[1] = 0.5;
        vertices.push_back(node);  // 11
        node[0] = 0.5;
        node[1] = 0.5;
        vertices.push_back(node);  // 12
        node[0] = 1.0;
        node[1] = 0.5;
        vertices.push_back(node);  // 13

        node[0] = 0.0;
        node[1] = 1.0;
        vertices.push_back(node);  // 14
        node[0] = 0.5;
        node[1] = 1.0;
        vertices.push_back(node);  // 15
      }
    ...
    DTM::pout << "\tn_global_active_cells = " << tria->n_global_active_cells()
              << std::endl
              << std::endl;
  ...
  if (TriaGenerator_Type.compare("backward_facing_step") == 0) {
    ...
    if (!dealii::Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)) {
      ...
      dealii::GridGenerator::subdivided_hyper_rectangle(
          channel_tria, repetitions, p1, p2,
          false  // colorize boundaries
      );
      ...
      dealii::GridGenerator::hyper_rectangle(step_tria, ps1, ps2, false);

      // merge step and channel
      dealii::GridGenerator::merge_triangulations(step_tria, channel_tria,
    ...
    DTM::pout << "DTM++: TriaGenerator: Successfully created backward facing "
                 "step grid."
              << std::endl;
  ...
  if (TriaGenerator_Type.compare("constrained_channel") == 0) {
    AssertThrow((dim == 2),
                dealii::ExcMessage(
                    "dim = 2 needed for constrained channel TriaGenerator"));

    AssertThrow(options.size() == 9 * 3,
                dealii::ExcMessage("TriaGenerator Options invalid, "
                                   "please check your input file data."));
    ...
    if (!dealii::Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)) {
      ...
      dealii::GridGenerator::subdivided_hyper_rectangle(
          inflow_channel, repetitions, p1, p2,
          false  // colorize boundaries
      );
      ...
      dealii::GridGenerator::subdivided_hyper_rectangle(
          constrained_channel, repetitions, p1, p2,
          false  // colorize boundaries
      );
      ...
      dealii::GridGenerator::subdivided_hyper_rectangle(
          outflow_channel, repetitions, p1, p2,
          false  // colorize boundaries
      );

      // merge
      dealii::GridGenerator::merge_triangulations(
          inflow_channel, constrained_channel, tria_tmp1);

      dealii::GridGenerator::merge_triangulations(tria_tmp1, outflow_channel,
                                                  tria_tmp2);
    ...
    DTM::pout << "DTM++: TriaGenerator: Successfully created constrained "
                 "channel with TriaGenerator."
              << std::endl;
  ...
  AssertThrow(
      false,
      dealii::ExcMessage(
          "TriaGenerator_Type unknown, please check your input file data."));
}
