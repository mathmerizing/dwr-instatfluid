...
Grid<dim>::~Grid() {
  ...
  for (; slab != ends; ++slab) {
    ...
    if (!parameter_set->fe.primal_order.compare("low") ||
      ...
      Assert(slab->space.low.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
      Assert(slab->time.low.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
    ...
    if (!parameter_set->fe.primal_order.compare("high") ||
      ...
      Assert(slab->space.high.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
      Assert(slab->time.high.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
    ...
    if ((slab->space.pu.fe_info->dof.use_count() > 0) ||
      ...
      Assert(slab->space.pu.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
      Assert(slab->time.pu.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
...
template <int dim>
void Grid<dim>::initialize_slabs() {
  ...
  numoftimeintervals = static_cast<unsigned int>(
      std::floor((parameter_set->time.fluid.T - parameter_set->time.fluid.t0) /
  ...
  for (unsigned int i{1}; i <= numoftimeintervals; ++i) {
    ...
    if (!parameter_set->primal_only || i == 1) {
      slab.space.tria =
          std::make_shared<dealii::parallel::distributed::Triangulation<dim>>(
              mpi_comm,
              typename dealii::parallel::distributed::Triangulation<dim>::
                  MeshSmoothing(dealii::parallel::distributed::Triangulation<
                                dim>::maximum_smoothing),
  ...
  // init temporal "grids" of each slab
  {
    unsigned int n{1};
    for (auto &slab : slabs) {
      slab.t_m = (n - 1) * parameter_set->time.fluid.tau_n +
                 parameter_set->time.fluid.t0;
      slab.t_n =
          n * parameter_set->time.fluid.tau_n + parameter_set->time.fluid.t0;
      ++n;
...
template <int dim>
void Grid<dim>::initialize_low_grid_components_on_slab(
  ...
  if (!parameter_set->primal_only || slab == this->slabs.begin()) {
    ...
    slab->space.low.fe_info->dof =
        std::make_shared<dealii::DoFHandler<dim>>(*slab->space.tria);

    // check FE configuration parameters
    Assert((!parameter_set->fe.low.convection.space_type.compare("cG")),
           dealii::ExcMessage(
               "low convection (Fluid velocity) fe must be of the cG family"));

    Assert((!parameter_set->fe.low.pressure.space_type.compare("cG")),
    ...
    if (parameter_set->fe.low.convection.p <=
      ...
      DTM::pout << "WARNING: you try to use an unstable low FE" << std::endl;
    }
    ...
    // create low fe system
    {
      ...
      {
        if (!(parameter_set->fe.low.convection.space_type_support_points
                  .compare("Gauss-Lobatto"))) {
          fe_quad_b = std::make_shared<dealii::QGaussLobatto<1>>(
              (parameter_set->fe.low.convection.p + 1));

          DTM::pout << "FE: (low) convection b: "
                    << "created QGaussLobatto<1> quadrature" << std::endl;
        }
      }
      ...
      {
        if (!(parameter_set->fe.low.pressure.space_type_support_points.compare(
                "Gauss-Lobatto"))) {
          fe_quad_p = std::make_shared<dealii::QGaussLobatto<1>>(
              (parameter_set->fe.low.pressure.p + 1));

          DTM::pout << "FE: (low) pressure p: "
                    << "created QGaussLobatto<1> quadrature" << std::endl;
        }
      }
      ...
      {
        if (!parameter_set->fe.low.convection.space_type.compare("cG")) {
          fe_b = std::make_shared<dealii::FE_Q<dim>>(*fe_quad_b);
        }

        AssertThrow(fe_b.use_count(),
      ...
      {
        if (!parameter_set->fe.low.pressure.space_type.compare("cG")) {
          fe_p = std::make_shared<dealii::FE_Q<dim>>(*fe_quad_p);
        }

        AssertThrow(fe_p.use_count(),
      ...
      DTM::pout << "fluid: created " << slab->space.low.fe_info->fe->get_name()
                << std::endl;
    }
    ...
    slab->space.low.fe_info->initial_constraints =
        std::make_shared<dealii::AffineConstraints<double>>();
    slab->space.low.fe_info->mapping =
    ...
    slab->space.low.fe_info->hanging_node_constraints =
        std::prev(slab)->space.low.fe_info->hanging_node_constraints;
    slab->space.low.fe_info->initial_constraints =
        std::prev(slab)->space.low.fe_info->initial_constraints;
    slab->space.low.fe_info->mapping =
  ...
  slab->time.low.fe_info->dof =
      std::make_shared<dealii::DoFHandler<1>>(*slab->time.tria);

  // check FE configuration parameters
  Assert((!parameter_set->fe.low.convection.time_type.compare("dG")),
  ...
  // create low fe system (time)
  {
    ...
    std::shared_ptr<dealii::Quadrature<1>> fe_quad_time_convection;
    {
      if (!(parameter_set->fe.low.convection.time_type_support_points.compare(
              "Gauss"))) {
        fe_quad_time_convection = std::make_shared<dealii::QGauss<1>>(
            (parameter_set->fe.low.convection.r + 1));

        DTM::pout << "FE time: (low) convection b: "
                  << "created QGauss<1> quadrature" << std::endl;
      } else if (!(parameter_set->fe.low.convection.time_type_support_points
                       .compare("Gauss-Lobatto"))) {
        if (parameter_set->fe.low.convection.r < 1) {
          fe_quad_time_convection = std::make_shared<QRightBox<1>>();
          DTM::pout << "FE time: (low) convection b: "
                    << "created QRightBox quadrature" << std::endl;
        } else {
          fe_quad_time_convection = std::make_shared<dealii::QGaussLobatto<1>>(
              (parameter_set->fe.low.convection.r + 1));
    ...
    AssertThrow(fe_quad_time_convection.use_count(),
                dealii::ExcMessage(
                    "FE time: (low) convection b support points invalid"));

    // create FE time
    {
      if (!parameter_set->fe.low.convection.time_type.compare("dG")) {
  ...
  slab->time.low.fe_info->mapping =
      std::make_shared<dealii::MappingQ<1>>(parameter_set->fe.low.convection.r);
}

template <int dim>
void Grid<dim>::initialize_high_grid_components_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  if (!parameter_set->primal_only || slab == this->slabs.begin()) {
    ...
    // check FE configuration parameters
    Assert((!parameter_set->fe.high.convection.space_type.compare("cG")),
           dealii::ExcMessage(
               "high convection (Fluid velocity) fe must be of the cG family"));

    Assert((!parameter_set->fe.high.pressure.space_type.compare("cG")),
    ...
    if (parameter_set->fe.high.convection.p <=
      ...
      DTM::pout << "WARNING: you try to use an unstable high FE" << std::endl;
    }
    ...
    // create high fe system
    {
      ...
      {
        if (!(parameter_set->fe.high.convection.space_type_support_points
                  .compare("Gauss-Lobatto"))) {
          fe_quad_z_b = std::make_shared<dealii::QGaussLobatto<1>>(
              (parameter_set->fe.high.convection.p + 1));

          DTM::pout << "FE: (high) convection z_b: "
                    << "created QGaussLobatto<1> quadrature" << std::endl;
        }
      }
      ...
      {
        if (!(parameter_set->fe.high.pressure.space_type_support_points.compare(
                "Gauss-Lobatto"))) {
          fe_quad_z_p = std::make_shared<dealii::QGaussLobatto<1>>(
              (parameter_set->fe.high.pressure.p + 1));

          DTM::pout << "FE: (high) pressure p: "
                    << "created QGaussLobatto<1> quadrature" << std::endl;
        }
      }
      ...
      {
        if (!parameter_set->fe.high.convection.space_type.compare("cG")) {
          fe_z_b = std::make_shared<dealii::FE_Q<dim>>(*fe_quad_z_b);
        }

        AssertThrow(fe_z_b.use_count(),
      ...
      {
        if (!parameter_set->fe.high.pressure.space_type.compare("cG")) {
          fe_z_p = std::make_shared<dealii::FE_Q<dim>>(*fe_quad_z_p);
        }

        AssertThrow(fe_z_p.use_count(),
      ...
      DTM::pout << "fluid: (high) created "
                << slab->space.high.fe_info->fe->get_name() << std::endl;
    }
    ...
    slab->space.high.fe_info->hanging_node_constraints =
        std::prev(slab)->space.high.fe_info->hanging_node_constraints;
    slab->space.high.fe_info->initial_constraints =
        std::prev(slab)->space.high.fe_info->initial_constraints;
    slab->space.high.fe_info->mapping =
  ...
  // create high fe system (time)
  {
    ...
    {
      if (!(parameter_set->fe.high.convection.time_type_support_points.compare(
              "Gauss"))) {
        fe_quad_time_high_convection = std::make_shared<dealii::QGauss<1>>(
            (parameter_set->fe.high.convection.r + 1));

        DTM::pout << "FE time: (high) convection b: "
                  << "created QGauss<1> quadrature" << std::endl;
      } else if (!(parameter_set->fe.high.convection.time_type_support_points
                       .compare("Gauss-Lobatto"))) {
        if (parameter_set->fe.high.convection.r < 1) {
          fe_quad_time_high_convection = std::make_shared<QRightBox<1>>();
          DTM::pout << "FE time: (high) convection b: "
                    << "created QRightBox quadrature" << std::endl;
        } else {
    ...
    AssertThrow(fe_quad_time_high_convection.use_count(),
                dealii::ExcMessage(
                    "FE time: (high) convection b support points invalid"));

    // create FE time
    {
      if (!parameter_set->fe.high.convection.time_type.compare("dG")) {
...
template <int dim>
void Grid<dim>::initialize_pu_grid_components_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  // create pu fe system
  {
    ...
    std::shared_ptr<dealii::Quadrature<1>> fe_quad;
    {
      ...
      DTM::pout << "FE: (PU) pressure p: "
                << "created QGaussLobatto<1> quadrature" << std::endl;
    }

    // create FE
    slab->space.pu.fe_info->fe = std::make_shared<dealii::FE_Q<dim>>(*fe_quad);

    DTM::pout << "stokes: (PU) created "
              << slab->space.pu.fe_info->fe->get_name() << std::endl;
  }
...
template <int dim>
void Grid<dim>::initialize_vorticity_grid_components_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  // create pu fe system
  {
    ...
    std::shared_ptr<dealii::Quadrature<1>> fe_quad;
    if (!(parameter_set->fe.low.convection.space_type_support_points.compare(
            "Gauss-Lobatto"))) {
    ...
    // create FE
    slab->space.vorticity.fe_info->fe =
        std::make_shared<dealii::FE_Q<dim>>(*fe_quad);

    DTM::pout << "FE: (vorticity) created "
              << slab->space.vorticity.fe_info->fe->get_name() << std::endl;
  }

  //	slab->space.vorticity.fe_info->constraints = std::make_shared<
  //dealii::AffineConstraints<double> > ();

  slab->space.vorticity.fe_info->mapping =
      std::make_shared<dealii::MappingQ<dim>>(1);
  ...
  // create vorticity fe system (time)
  {
    ...
    std::shared_ptr<dealii::Quadrature<1>> fe_quad_time_vorticity;
    {
      if (!(parameter_set->fe.low.convection.time_type_support_points.compare(
              "Gauss"))) {
        fe_quad_time_vorticity = std::make_shared<dealii::QGauss<1>>(
            (parameter_set->fe.low.convection.r + 1));
        ...
        DTM::pout << "FE time: (low) convection b: "
                  << "created QGauss<1> quadrature" << std::endl;
                       ...
                       .compare("Gauss-Lobatto"))) {
        if (parameter_set->fe.low.convection.r < 1) {
          fe_quad_time_vorticity = std::make_shared<QRightBox<1>>();
          DTM::pout << "FE time: (low) convection b: "
                    << "created QRightBox quadrature" << std::endl;
        } else {
          fe_quad_time_vorticity = std::make_shared<dealii::QGaussLobatto<1>>(
              (parameter_set->fe.low.convection.r + 1));
  ...
  slab->time.vorticity.fe_info->mapping =
      std::make_shared<dealii::MappingQ<1>>(1);
}

template <int dim>
bool Grid<dim>::split_slab_in_time(
  ...
  // check if iterator slab is in the container slabs of this object
  {
    ...
    Assert(check,
           dealii::ExcMessage("your given iterator slab to be refined could "
                              "not be found in this->slabs object"));
  }
#endif
  ...
  if (M <= parameter_set->time.fluid.max_intervals_per_slab) return false;
  ...
  dealii::GridGenerator::subdivided_hyper_rectangle(*slab->time.tria, spacing_1,
                                                    p1_1, p2_1);
  ...
  dealii::GridGenerator::subdivided_hyper_rectangle(*std::prev(slab)->time.tria,
                                                    spacing_2, p1_2, p2_2);
  ...
  if (!parameter_set->fe.primal_order.compare("low")) {
    ...
    AssertThrow(
        false, dealii::ExcMessage("primal_order needs to be 'low' or 'high'."));
  }
...
template <int dim>
void Grid<dim>::refine_slab_in_time(
    typename fluid::types::spacetime::dwr::slabs<dim>::iterator slab) {
  Assert(false,
         dealii::ExcMessage("This function should not be in use anymore!"));
}

/// Generate tria on each slab.
template <int dim>
void Grid<dim>::generate() {
  coarse_tria = std::make_shared<dealii::Triangulation<dim>>(
      typename dealii::Triangulation<dim>::MeshSmoothing(
          dealii::Triangulation<dim>::smoothing_on_refinement));
  fluid::TriaGenerator<dim> tria_generator;
  tria_generator.generate(parameter_set->TriaGenerator,
                          parameter_set->TriaGenerator_Options, coarse_tria);

  auto slab(this->slabs.begin());
  auto ends(this->slabs.end());
  slab->space.tria->copy_triangulation(*coarse_tria);
  dealii::GridGenerator::hyper_cube(*slab->time.tria, slab->t_m, slab->t_n,
                                    false);
  slab++;
  for (; slab != ends; ++slab) {
    if (!parameter_set->primal_only || slab == this->slabs.begin())
...
/// Global refinement.
template <int dim>
void Grid<dim>::refine_global(const unsigned int &space_n,
  ...
  while (was_split) {
    ...
    was_split = false;
    for (; slab != ends; ++slab) {
      if (split_slab_in_time(slab)) was_split = true;
    }
  }
}
...
template <int dim>
void Grid<dim>::set_manifolds() {
  // base class: do nothing
}

/// Set boundary indicators
template <int dim>
void Grid<dim>::set_boundary_indicators() {
  // base class does not implement this function
  Assert(false, dealii::ExcNotImplemented());
}

template <int dim>
void Grid<dim>::set_dirichlet_function(
    std::shared_ptr<dealii::TensorFunction<1, dim, double>> fun) {
  dirichlet_function = fun;
}

template <int dim>
void Grid<dim>::distribute() {
  // distribute is now being performed slab wise
  Assert(false, dealii::ExcNotImplemented());
}

template <int dim>
void Grid<dim>::distribute_low_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ////////////////////////////////////////////////////////////////////////
  // space
  ...
  if (!parameter_set->primal_only || slab == this->slabs.begin()) {
    Assert(slab->space.low.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());
    Assert(slab->space.low.fe_info->fe.use_count(),
           dealii::ExcNotInitialized());
    slab->space.low.fe_info->dof->distribute_dofs(
        *(slab->space.low.fe_info->fe));
    ...
    dofs_per_component = dealii::DoFTools::count_dofs_per_fe_component(
        *slab->space.low.fe_info->dof, true);
    ...
    DTM::pout << "\tn_dofs (low)       = "
              << slab->space.low.fe_info->dof->n_dofs() << std::endl;
    ...
    Assert(slab->space.low.fe_info->constraints.use_count(),
           dealii::ExcNotInitialized());
    {
      ...
      slab->space.low.fe_info->hanging_node_constraints->reinit(
          *slab->space.low.fe_info->locally_relevant_dofs);

      slab->space.low.fe_info->initial_constraints->clear();
      slab->space.low.fe_info->initial_constraints->reinit(
          *slab->space.low.fe_info->locally_relevant_dofs);

      interpolate_dirichlet_bc(slab->space.low.fe_info->dof,
                               slab->space.low.fe_info->constraints, slab->t_m,
                               false);
      ...
      dealii::DoFTools::make_hanging_node_constraints(
          *slab->space.low.fe_info->dof, *slab->space.low.fe_info->constraints);
      dealii::DoFTools::make_hanging_node_constraints(
          *slab->space.low.fe_info->dof,
          *slab->space.low.fe_info->hanging_node_constraints);
      dealii::DoFTools::make_hanging_node_constraints(
    ...
    slab->space.low.fe_info->locally_owned_dofs =
        std::prev(slab)->space.low.fe_info->locally_owned_dofs;
    slab->space.low.fe_info->locally_relevant_dofs =
        std::prev(slab)->space.low.fe_info->locally_relevant_dofs;
    slab->space.low.fe_info->constraints =
        std::prev(slab)->space.low.fe_info->constraints;
  }
...
template <int dim>
void Grid<dim>::distribute_high_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ////////////////////////////////////////////////////////////////////////
  // space
  ...
  if (!parameter_set->primal_only || slab == this->slabs.begin()) {
    Assert(slab->space.high.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());
    Assert(slab->space.high.fe_info->fe.use_count(),
           dealii::ExcNotInitialized());
    slab->space.high.fe_info->dof->distribute_dofs(
    ...
    dofs_per_component = dealii::DoFTools::count_dofs_per_fe_component(
        *slab->space.high.fe_info->dof, true);
    ...
    DTM::pout << "\tn_dofs (high)         = "
              << slab->space.high.fe_info->dof->n_dofs() << std::endl;
    ...
    // setup constraints (e.g. hanging nodes)
    Assert(slab->space.high.fe_info->constraints.use_count(),
           dealii::ExcNotInitialized());
    {
      ...
      slab->space.high.fe_info->hanging_node_constraints->reinit(
          *slab->space.high.fe_info->locally_relevant_dofs);

      slab->space.high.fe_info->initial_constraints->clear();
      slab->space.high.fe_info->initial_constraints->reinit(
          *slab->space.high.fe_info->locally_relevant_dofs);
      interpolate_dirichlet_bc(slab->space.high.fe_info->dof,
                               slab->space.high.fe_info->constraints, slab->t_m,
                               false);
    ...
    slab->space.high.fe_info->locally_owned_dofs =
        std::prev(slab)->space.high.fe_info->locally_owned_dofs;
    slab->space.high.fe_info->locally_relevant_dofs =
        std::prev(slab)->space.high.fe_info->locally_relevant_dofs;
    slab->space.high.fe_info->constraints =
        std::prev(slab)->space.high.fe_info->constraints;
  }
  ...
  // distribute high dofs
  {
    Assert(slab->time.high.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());
    Assert(slab->time.high.fe_info->fe.use_count(),
           dealii::ExcNotInitialized());
    slab->time.high.fe_info->dof->distribute_dofs(
        *(slab->time.high.fe_info->fe));
  }
}

template <int dim>
void Grid<dim>::distribute_pu_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ////////////////////////////////////////////////////////////////////////
  // space
  // distribute pu dofs and create constraints
  {
    ...
    DTM::pout << "\tn_dofs (PU)         = "
              << slab->space.pu.fe_info->dof->n_dofs() << std::endl;
    ...
    // setup constraints (e.g. hanging nodes)
    Assert(slab->space.pu.fe_info->constraints.use_count(),
           dealii::ExcNotInitialized());
    {
      ...
      Assert(slab->space.pu.fe_info->dof.use_count(),
             dealii::ExcNotInitialized());
      dealii::DoFTools::make_hanging_node_constraints(
          *slab->space.pu.fe_info->dof, *slab->space.pu.fe_info->constraints);
...
template <int dim>
void Grid<dim>::distribute_vorticity_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ////////////////////////////////////////////////////////////////////////
  // space
  // distribute pu dofs and create constraints
  {
    Assert(slab->space.vorticity.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());
    Assert(slab->space.vorticity.fe_info->fe.use_count(),
           dealii::ExcNotInitialized());
    slab->space.vorticity.fe_info->dof->distribute_dofs(
        *(slab->space.vorticity.fe_info->fe));

    DTM::pout << "\tn_dofs (vorticity)         = "
              << slab->space.vorticity.fe_info->dof->n_dofs() << std::endl;
    ...
    slab->space.vorticity.fe_info->locally_owned_dofs =
        std::make_shared<dealii::IndexSet>(
            slab->space.vorticity.fe_info->dof->locally_owned_dofs());
  ...
  // distribute vorticity dofs
  {
    Assert(slab->time.vorticity.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());
    Assert(slab->time.vorticity.fe_info->fe.use_count(),
           dealii::ExcNotInitialized());
    slab->time.vorticity.fe_info->dof->distribute_dofs(
        *(slab->time.vorticity.fe_info->fe));
  }
}
template <int dim>
void Grid<dim>::interpolate_dirichlet_bc(
    std::shared_ptr<dealii::DoFHandler<dim>> dof,
    std::shared_ptr<dealii::AffineConstraints<double>> constraints, double tm,
    bool inhom) {
  auto component_mask_convection = dealii::ComponentMask((dim + 1), true);
  component_mask_convection.set(dim, false);

  if (inhom) {
    dirichlet_function->set_time(tm);
    std::shared_ptr<dealii::VectorFunctionFromTensorFunction<dim>> bc_fun =
        std::make_shared<dealii::VectorFunctionFromTensorFunction<dim>>(
            *dirichlet_function, 0, (dim + 1));
    ...
    dealii::VectorTools::interpolate_boundary_values(
        ...
        *bc_fun, *constraints, component_mask_convection);
    dealii::VectorTools::interpolate_boundary_values(
        *dof,
        static_cast<dealii::types::boundary_id>(
            fluid::types::space::boundary_id::prescribed_convection_c2 +
    ...
    dealii::VectorTools::interpolate_boundary_values(
        *dof,
        static_cast<dealii::types::boundary_id>(
            ...
            fluid::types::space::boundary_id::prescribed_convection_c2 +
            fluid::types::space::boundary_id::prescribed_convection_c1),
        dealii::ZeroFunction<dim>(dim + 1), *constraints,
...
template <int dim>
void Grid<dim>::create_sparsity_pattern_primal_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  // pattern for initial constraints (for projection only)
  {
...

    {
      ...
      dealii::SparsityTools::distribute_sparsity_pattern(
          dsp, *slab->space.primal.fe_info->locally_owned_dofs, mpi_comm,
          *slab->space.primal.fe_info->locally_relevant_dofs);

      L_space->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
                      *slab->space.primal.fe_info->locally_owned_dofs, dsp);
    }
  }
  ////////////////////////////////////////////////////////////////////////////
  // space-time
  ...
  // primal sparsity pattern (downwind test function in time)
  {
    ...
    dealii::DynamicSparsityPattern dsp(
        slab->space.primal.fe_info->dof->n_dofs() *
            slab->time.primal.fe_info->dof->n_dofs(),  // test
        slab->space.primal.fe_info->dof->n_dofs() *
            slab->time.primal.fe_info->dof->n_dofs()  // trial
    );

    {
      ...
      {
        Assert(slab->space.primal.sp_L.use_count(),
               dealii::ExcNotInitialized());
        ...
        for (; cell != endc; ++cell) {
          for (dealii::types::global_dof_index nn{0};
            ...
            offset = slab->space.primal.fe_info->dof->n_dofs() * nn *
                     slab->time.primal.fe_info->fe->dofs_per_cell;

            if (nn)
              for (unsigned int i{0};
                   i < slab->time.primal.fe_info->fe->dofs_per_cell; ++i)
                for (unsigned int j{0};
                     j < slab->time.primal.fe_info->fe->dofs_per_cell; ++j) {
                  dsp.add(
                      // downwind test
                      cell->row() + offset +
    ...
    L_spacetime->reinit(*slab->spacetime.primal.locally_owned_dofs,
                        *slab->spacetime.primal.locally_owned_dofs, dsp);
    //		}
  }
...
template <int dim>
void Grid<dim>::create_sparsity_pattern_dual_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  // dual sparsity pattern (upwind trial function in time)
  {
    ...
    dealii::DynamicSparsityPattern dual_dsp(
        slab->space.dual.fe_info->dof->n_dofs() *
            slab->time.dual.fe_info->dof->n_dofs(),  // test
        slab->space.dual.fe_info->dof->n_dofs() *
            slab->time.dual.fe_info->dof->n_dofs()  // trial
    );
    ...
    dofs_per_component = dealii::DoFTools::count_dofs_per_fe_component(
        *slab->space.dual.fe_info->dof, true);
...

    {
      ...
      // term (trace operator) on the test functions
      {
        ...
        for (; cell != endc; ++cell) {
          for (dealii::types::global_dof_index nn{0};
               nn < slab->time.tria->n_global_active_cells(); ++nn) {
            offset = slab->space.dual.fe_info->dof->n_dofs() * nn *
                     slab->time.dual.fe_info->fe->dofs_per_cell;

            if (nn)
              for (unsigned int i{0};
                   i < slab->time.dual.fe_info->fe->dofs_per_cell; ++i)
                for (unsigned int j{0};
                     j < slab->time.dual.fe_info->fe->dofs_per_cell; ++j) {
                  dual_dsp.add(
                      // test
                      cell->row() + offset -
                          slab->space.dual.fe_info->dof->n_dofs() *
    ...
    dealii::SparsityTools::distribute_sparsity_pattern(
        dual_dsp, *slab->spacetime.dual.locally_owned_dofs, mpi_comm,
        *slab->spacetime.dual.locally_relevant_dofs);

    L->reinit(*slab->spacetime.dual.locally_owned_dofs,
              *slab->spacetime.dual.locally_owned_dofs, dual_dsp);
  }
}

template <int dim>
void Grid<dim>::clear_primal_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ...
  // space
  DTM::pout
      << "         slab->space.primal.fe_info->dof->memory_consumption() = "
      << std::right << std::setw(20)
      << (slab->space.primal.fe_info->dof->memory_consumption() / 1000) << " KB"
      << std::endl;
  DTM::pout
      << "         slab->space.primal.fe_info->fe->memory_consumption()  = "
      << std::setw(20)
      << (slab->space.primal.fe_info->fe->memory_consumption() / 1000) << " KB"
      << std::endl;
  DTM::pout
      << "         "
         "slab->space.primal.fe_info->constraints->memory_consumption()  = "
      << std::setw(20)
      << (slab->space.primal.fe_info->constraints->memory_consumption() / 1000)
      << " KB" << std::endl;
  ...
  // time
  DTM::pout
      << "         slab->time.primal.fe_info->dof->memory_consumption()  = "
      << std::setw(20)
      << (slab->time.primal.fe_info->dof->memory_consumption() / 1000) << " KB"
      << std::endl;
  DTM::pout
      << "         slab->time.primal.fe_info->fe->memory_consumption()   = "
      << std::setw(20)
      << (slab->time.primal.fe_info->fe->memory_consumption() / 1000) << " KB"
      << std::endl;
...
template <int dim>
void Grid<dim>::clear_dual_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  Assert(slab->time.dual.fe_info->dof.use_count(), dealii::ExcNotInitialized());

  // space
  DTM::pout << "         slab->space.dual.fe_info->dof->memory_consumption() = "
            << std::right << std::setw(20)
            << (slab->space.dual.fe_info->dof->memory_consumption() / 1000)
            << " KB" << std::endl;
  DTM::pout << "         slab->space.dual.fe_info->fe->memory_consumption()  = "
            << std::setw(20)
            << (slab->space.dual.fe_info->fe->memory_consumption() / 1000)
            << " KB" << std::endl;
  DTM::pout << "         "
               "slab->space.dual.fe_info->constraints->memory_consumption()  = "
            << std::setw(20)
            << (slab->space.dual.fe_info->constraints->memory_consumption() /
                1000)
            << " KB" << std::endl;
  ...
  // time
  DTM::pout << "         slab->time.dual.fe_info->dof->memory_consumption()  = "
            << std::setw(20)
            << (slab->time.dual.fe_info->dof->memory_consumption() / 1000)
            << " KB" << std::endl;
  DTM::pout << "         slab->time.dual.fe_info->fe->memory_consumption()   = "
            << std::setw(20)
            << (slab->time.dual.fe_info->fe->memory_consumption() / 1000)
            << " KB" << std::endl;
