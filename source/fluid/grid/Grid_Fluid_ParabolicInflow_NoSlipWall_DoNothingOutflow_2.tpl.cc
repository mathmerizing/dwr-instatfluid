/**
 * @file Grid_Fluid_ParabolicInflow_NoSlipWall_DoNothingOutflow_2.tpl.cc
...
template <int dim>
void Grid_Fluid_ParabolicInflow_NoSlipWall_DoNothingOutflow_2<
    dim>::set_boundary_indicators() {
        ...
        if (cell->at_boundary()) {
          for (unsigned int face(0);
            ...
            if (cell->face(face)->at_boundary()) {
              ...
              if ((std::abs(center[0] - x_in) < 1e-12)) {
                // inflow boundary
              ...
              } else if ((std::abs(center[0] - x_out) < 1e-12)) {
                // outflow boundary
                cell->face(face)->set_boundary_id(static_cast<
                                                  dealii::types::boundary_id>(
...
#include "Grid_Fluid_ParabolicInflow_NoSlipWall_DoNothingOutflow_2.inst.in"
