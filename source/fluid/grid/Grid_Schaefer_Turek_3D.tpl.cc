...
template <int dim>
void Grid_Schaefer_Turek_3D<dim>::set_manifolds() {
...
template <int dim>
void Grid_Schaefer_Turek_3D<dim>::set_obstacle_manifold(
    ...
    if (cell->at_boundary())
      for (unsigned int face(0);
        ...
        if (cell->face(face)->at_boundary()) {
          ...
          double distance = std::sqrt((center(0) - x_mp) * (center(0) - x_mp) +
          ...
          if (distance < 0.055  // radius of 0.05 + Tolerance
            ...
            cell->face(face)->set_manifold_id(42  // CylindricalManifold
            );
          }
        }
      }

  tria->set_manifold(42,
                     dealii::CylindricalManifold<dim>(z_dir, p_obstacle_mid));
}
template <int dim>
void Grid_Schaefer_Turek_3D<dim>::set_boundary_indicators() {
        ...
        if (cell->at_boundary())
          for (unsigned int face(0);
            ...
            if (cell->face(face)->at_boundary()) {
              ...
              // walls: no-flow / no-slip conditions for y=0, z=0 and y=0.41,
              // z=0.41 prescribed homog. Dirichlet for convection
