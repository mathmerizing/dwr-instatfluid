...
template <int dim>
void Selector<dim>::create_function(
    const std::string &_type, const std::string &_options,
    std::shared_ptr<dealii::TensorFunction<1, dim> > &tensor_function) const {
  ////////////////////////////////////////////////////////////////////////////
  ...
  DTM::pout << "* found configuration: convection dirichlet function = "
            << _type << std::endl;
  DTM::pout << "* found configuration: convection dirichlet options = "
            << std::endl;
  ...
  if (_type.compare("ZeroTensorFunction") == 0) {
    AssertThrow(options.size() == 0,
                dealii::ExcMessage("convection dirichlet options invalid, "
                                   "please check your input file data."));

    tensor_function =
        std::make_shared<dealii::ZeroTensorFunction<1, dim, double> >();

    DTM::pout << "convection dirichlet selector: created zero tensor function"
              << std::endl
              << std::endl;
  ...
  if (_type.compare("ConstantTensorFunction") == 0) {
    ...
    tensor_function =
        std::make_shared<dealii::ConstantTensorFunction<1, dim, double> >(f);
  ...
  if (_type.compare("Convection_Parabolic_Inflow_1") == 0) {
    ...
    DTM::pout << "convection dirichlet selector: "
              << "created parabolic inflow 1 tensor function, with" << std::endl
              << "\tscaling y-component = " << std::stod(options.at(0)) << " . "
              << std::endl
              << std::endl;
  ...
  if (_type.compare("Convection_Parabolic_Inflow_1_sin") == 0) {
    ...
    DTM::pout << "convection dirichlet selector: "
              << "created parabolic inflow 1 sin tensor function, with"
              << std::endl
              << "\tscaling y-component = " << std::stod(options.at(0)) << " . "
              << std::endl
              << std::endl;
  ...
  if (_type.compare("Convection_Parabolic_Inflow_2") == 0) {
    ...
    DTM::pout << "convection dirichlet selector: "
              ...
              << "\tscaling y-component = " << std::stod(options.at(2)) << " . "
              << std::endl
              << std::endl;
  ...
  if (_type.compare("Convection_Parabolic_Inflow_3_sin") == 0) {
    ...
    DTM::pout << "convection dirichlet selector: "
              << "created parabolic inflow 3 sin tensor function, with"
              << std::endl
              << std::endl;
  ...
  //
  AssertThrow(false,
              dealii::ExcMessage("convection dirichlet function unknown, "
                                 "please check your input file data."));
}
