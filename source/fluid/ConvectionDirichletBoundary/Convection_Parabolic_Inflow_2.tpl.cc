...
template <int dim>
Parabolic_Inflow_2<dim>::Parabolic_Inflow_2(const double &y_min,
                                            const double &y_max,
  ...
  for (unsigned int i{0}; i < 3; ++i) a_y[i] = x(i) * scaling;
}

template <int dim>
dealii::Tensor<1, dim> Parabolic_Inflow_2<dim>::value(
