...
template <int dim>
using Je_MeanDrag_Assembler =
    goal::mean::drag::spacetime::Operator::Assembler<dim>;

#include <fluid/assembler/ST_Dual_MeanVorticityAssembly.tpl.hh>
template <int dim>
using Je_MeanVorticity_Assembler =
    goal::mean::vorticity::spacetime::Operator::Assembler<dim>;
...
template <int dim>
using ProjectionRHSAssembler =
    projectionrhs::spacetime::Operator::Assembler<dim>;

// dual divergence free projection
//#include <fluid/assembler/ST_Dual_DivFreeProjectionAssembly.tpl.hh>
// template <int dim>
// using DualProjectionAssembler =
// projection::spacetime::dual::Operator::Assembler<dim>; #include
//<fluid/assembler/ST_Dual_DivFreeProjectionRHSAssembly.tpl.hh> template <int
// dim> using DualProjectionRHSAssembler =
// projectionrhs::spacetime::dual::Operator::Assembler<dim>;
...
void Fluid<dim>::run() {
  ...
  DTM::pout << "solving only primal problem: "
            << (parameter_set->primal_only ? "true" : "false") << std::endl;

  // check problem we want to solve
  if (parameter_set->problem.compare("Stokes") == 0) {
    DTM::pout << "solving the linear Stokes equations" << std::endl;
  } else if (parameter_set->problem.compare("Navier-Stokes") == 0) {
    DTM::pout << "solving the quasilinear Navier-Stokes equations\n"
              << std::endl;
  } else {
    AssertThrow(false,
                dealii::ExcMessage(
  ...
  DTM::pout << "symmetric stress: "
            << (parameter_set->fe.symmetric_stress ? "true" : "false")
            << std::endl;
  ...
  if ((parameter_set->fe.primal.convection.space_type.compare("cG") == 0) &&
    ...
    DTM::pout << "primal space discretisation convection-pressure:" << std::endl
              << "\t[ "
              // convection
              << "cG(" << parameter_set->fe.primal.convection.p << ")-Q_"
              << parameter_set->fe.primal.convection.space_type_support_points
              << ", "
              // pressure
              << "cG(" << parameter_set->fe.primal.pressure.p << ")-Q_"
              << parameter_set->fe.primal.pressure.space_type_support_points
              << " ]^T" << std::endl;
  } else {
    AssertThrow(false,
                dealii::ExcMessage("primal space discretisation unknown"));
  }
  ...
  if ((parameter_set->fe.primal.convection.time_type.compare("dG") == 0) &&
    ...
    DTM::pout << "primal time discretisation convection-pressure:" << std::endl
              << "\t[ "
              // convection
              << "dG(" << parameter_set->fe.primal.convection.r << ")-Q_"
              << parameter_set->fe.primal.convection.time_type_support_points
              << ", "
              // pressure
              << "dG(" << parameter_set->fe.primal.pressure.r << ")-Q_"
  ...
  if ((parameter_set->fe.dual.convection.space_type.compare("cG") == 0) &&
    ...
    DTM::pout << "dual space discretisation convection-pressure:" << std::endl
              << "\t[ "
              // convection
              << "cG(" << parameter_set->fe.dual.convection.p << ")-Q_"
              << parameter_set->fe.dual.convection.space_type_support_points
              << ", "
              // pressure
              << "cG(" << parameter_set->fe.dual.pressure.p << ")-Q_"
              << parameter_set->fe.dual.pressure.space_type_support_points
              << " ]^T" << std::endl;
  } else {
    AssertThrow(false, dealii::ExcMessage("dual space discretisation unknown"));
  }
  ...
  if ((parameter_set->fe.dual.convection.time_type.compare("dG") == 0) &&
    ...
    DTM::pout << "dual time discretisation convection-pressure:" << std::endl
              << "\t[ "
              // convection
              << "dG(" << parameter_set->fe.dual.convection.r << ")-Q_"
              << parameter_set->fe.dual.convection.time_type_support_points
              << ", "
              // pressure
              << "dG(" << parameter_set->fe.dual.pressure.r << ")-Q_"
              << parameter_set->fe.dual.pressure.time_type_support_points
              << " ]^T" << std::endl;
  } else {
    AssertThrow(false, dealii::ExcMessage("dual time discretisation unknown"));
  }
  DTM::pout << std::endl << std::endl;

  // give info about functional values
  DTM::pout << "The following functional values will be calculated:"
            << std::endl;
  ...
  if (!parameter_set->dwr.goal.type.compare("mean_drag")) {
    DTM::pout << "mean drag coefficient" << std::endl;
    AssertThrow(parameter_set->dwr.functional.mean_drag,
                dealii::ExcMessage("You want to refine based on a functional "
                                   "that is not calculated, aborting!"));
  } else if (!parameter_set->dwr.goal.type.compare("mean_lift")) {
    AssertThrow(false, dealii::ExcNotImplemented());
    DTM::pout << "mean lift coefficient" << std::endl;
    AssertThrow(parameter_set->dwr.functional.mean_lift,
                dealii::ExcMessage("You want to refine based on a functional "
                                   "that is not calculated, aborting!"));
  } else if (!parameter_set->dwr.goal.type.compare("mean_pdiff")) {
    AssertThrow(false, dealii::ExcNotImplemented());
    DTM::pout << "mean pressure difference" << std::endl;
    AssertThrow(parameter_set->dwr.functional.mean_pdiff,
                dealii::ExcMessage("You want to refine based on a functional "
                                   "that is not calculated, aborting!"));
  } else if (!parameter_set->dwr.goal.type.compare("mean_vorticity")) {
    DTM::pout << "mean vorticity" << std::endl;
    AssertThrow(parameter_set->dwr.functional.mean_vorticity,
                dealii::ExcMessage("You want to refine based on a functional "
                                   "that is not calculated, aborting!"));
  }
  ...
  for (; slab != ends; ++slab) {
    ...
    if (!parameter_set->fe.primal_order.compare("low")) {
      ...
      AssertThrow(false, dealii::ExcMessage(
                             "primal_order needs to be 'low' or 'high'."));
    }
    ...
    if (!parameter_set->fe.dual_order.compare("low")) {
      ...
      AssertThrow(
          false, dealii::ExcMessage(
                     "dual_order needs to be 'low' or 'high' or 'high-time'."));
    }
  }  // end for-loop slab
  ...
  do {
    if (dwr_loop > 1) {
      if (!parameter_set->dwr.refine_and_coarsen.spacetime.strategy.compare(
              "global_space")) {
        // refine grid in space
        grid->refine_global(1, 0);
      } else if (!parameter_set->dwr.refine_and_coarsen.spacetime.strategy
                      .compare("global_time")) {
        // refine grid in time
        ...
        AssertThrow(
            ...
            dealii::ExcMessage("need to solve dual problem and assemble error "
                               "estimator to do adaptive mesh refinement"));
    ...
    auto ends{grid->slabs.end()};
    for (; slab != ends; ++slab) {
      n_primal_st_dofs += slab->space.primal.fe_info->dof->n_dofs() *
                          slab->time.primal.fe_info->dof->n_dofs();
      if (!parameter_set->primal_only)
        n_dual_st_dofs += slab->space.dual.fe_info->dof->n_dofs() *
...
void Fluid<dim>::init_grid() {
  ...
  //
  {
    ...
    selector.create_grid(parameter_set, grid);
  ...
  Assert((parameter_set->time.fluid.t0 < parameter_set->time.fluid.T),
  ...
  grid->refine_global(parameter_set->space.mesh.fluid.global_refinement,
                      parameter_set->time.fluid.initial_time_tria_refinement);

  DTM::pout << "grid: number of slabs = " << grid->slabs.size() << std::endl;

  Assert(function.convection.dirichlet.use_count(),
         dealii::ExcNotInitialized());
  grid->set_dirichlet_function(function.convection.dirichlet);
}

template <int dim>
void Fluid<dim>::init_functions() {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  // viscosity function:
  {
    fluid::viscosity::Selector<dim> selector;
    selector.create_function(parameter_set->viscosity_function,
  ...
  // fluid force function:
  {
    ...
    selector.create_function(parameter_set->fluid.force_function,
...
template <int dim>
void Fluid<dim>::init_reference_values() {
...
template <int dim>
void Fluid<dim>::init_newton_parameters() {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  primal.sc = std::make_shared<dealii::SolverControl>(
      10000, newton.lower_bound * 1.0e-08, false, false);
  primal.ad =
      std::make_shared<dealii::TrilinosWrappers::SolverDirect::AdditionalData>(
          false, "Amesos_Mumps");

  dual.sc = std::make_shared<dealii::SolverControl>(
      10000, newton.lower_bound * 1.0e-08, false, false);
  dual.ad =
      std::make_shared<dealii::TrilinosWrappers::SolverDirect::AdditionalData>(
          false, "Amesos_Mumps");
...
template <int dim>
void Fluid<dim>::primal_reinit_storage() {
  ////////////////////////////////////////////////////////////////////////////
  ...
  primal.storage.u->resize(static_cast<unsigned int>(grid->slabs.size()));

  primal.storage.um =
      std::make_shared<DTM::types::storage_data_trilinos_vectors<1>>();

  primal.storage.um->resize(static_cast<unsigned int>(grid->slabs.size()));
...
template <int dim>
void Fluid<dim>::primal_reinit_storage_on_slab(
  ...
  for (unsigned int j{0}; j < x->x.size(); ++j) {
    ...
    x->x[j]->reinit(*slab->spacetime.primal.locally_owned_dofs,
                    *slab->spacetime.primal.locally_relevant_dofs, mpi_comm);

    // xm
    xm->x[j] = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();

    xm->x[j]->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
...
template <int dim>
void Fluid<dim>::primal_reinit_vorticity_storage_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  for (unsigned int j{0}; j < v->x.size(); ++j) {
    ...
    Assert(slab->time.vorticity.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());

    v->x[j]->reinit(*slab->spacetime.vorticity.locally_owned_dofs,
                    *slab->spacetime.vorticity.locally_relevant_dofs, mpi_comm);
  }
}

template <int dim>
void Fluid<dim>::primal_assemble_system(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  *primal.L = 0;
  ...
  {
    fluid::spacetime::Operator::Assembler<dim> assembler;

    assembler.set_symmetric_stress(parameter_set->fe.symmetric_stress);

    Assert(function.viscosity.use_count(), dealii::ExcNotInitialized());
    assembler.set_functions(function.viscosity);
    ...
    assembler.assemble(primal.L, slab, u,
                       (parameter_set->problem.compare("Navier-Stokes") == 0));
  }
}

template <int dim>
void Fluid<dim>::primal_assemble_const_rhs(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  if (!parameter_set->fe.primal_projection_type.compare("none")) {
    ...
    *primal.projection_matrix = 0;
    ...
    primal.projection_rhs->reinit(
        *slab->space.primal.fe_info->locally_owned_dofs, mpi_comm);
    *primal.projection_rhs = 0;

    primal.um_projected =
        std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
    primal.um_projected->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
                                mpi_comm);
    *primal.um_projected = 0;

    auto tmp_relevant =
        std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
    tmp_relevant->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
                         *slab->space.primal.fe_info->locally_relevant_dofs,
                         mpi_comm);
    *tmp_relevant = *primal.um;
    ...
    {
      DTM::pout << "dwr-instatfluid: divergence free projection (gradient="
                << (use_gradient_projection ? "true" : "false") << ")...";

      // 1. assemble projection matrix
      ProjectionAssembler<dim> matrix_assembler;
      matrix_assembler.set_gradient_projection(
          use_gradient_projection);  // TRUE: use H^1_0 projection; FALSE: use
                                     // L^2 projection

      Assert(primal.projection_matrix.use_count(), dealii::ExcNotInitialized());
      matrix_assembler.assemble(primal.projection_matrix, slab);

      // 2. assemble projection rhs
      ProjectionRHSAssembler<dim> rhs_assembler;
      rhs_assembler.set_gradient_projection(
          use_gradient_projection);  // TRUE: use H^1_0 projection; FALSE: use
                                     // L^2 projection

      Assert(primal.um.use_count(), dealii::ExcNotInitialized());
      Assert(primal.projection_rhs.use_count(), dealii::ExcNotInitialized());
      rhs_assembler.assemble(tmp_relevant, primal.projection_rhs, slab);

      // 3. solve projection linear system for primal.um_projected
      primal.projection_iA =
          std::make_shared<dealii::TrilinosWrappers::SolverDirect>(*primal.sc,
                                                                   *primal.ad);
  ...
  primal.Mum->reinit(*slab->spacetime.primal.locally_owned_dofs, mpi_comm);
  *primal.Mum = 0.;

  {
    ...
    *primal.relevant_tmp = *primal.um;
    ...
    DTM::pout
        << "dwr-instatfluid: assemble space-time slab initial value vector...";
    Assert(primal.um.use_count(), dealii::ExcNotInitialized());
    Assert(primal.Mum.use_count(), dealii::ExcNotInitialized());
    assembler.assemble(um->x[0], primal.Mum, slab);

    DTM::pout << " (done)" << std::endl;
  }

  //	// ASSEMBLY SPACE-TIME OPERATOR: FORCE VECTOR
  //////////////////////////////// 	primal.f = std::make_shared<
  ...
  //
  //		Assert(function.fluid.force.use_count(),
  //dealii::ExcNotInitialized()); 		assembler.set_functions( 			function.fluid.force
  //		);
  //
  //		DTM::pout << "dwr-fluid: assemble space-time slab force
  //vector..."; 		Assert(primal.f.use_count(), dealii::ExcNotInitialized());
  //		assembler.assemble(
  //			primal.f,
  //			slab
  //		);
...
template <int dim>
void Fluid<dim>::primal_assemble_and_construct_Newton_rhs(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> u
  ...
  primal.Fu->reinit(*slab->spacetime.primal.locally_owned_dofs, mpi_comm);
  *primal.b = 0.;
  *primal.Fu = 0.;
  {
    ...
    assembler.assemble(primal.Fu, slab, u,
                       (parameter_set->problem.compare("Navier-Stokes") == 0));
  }

  *primal.b = *primal.Mum;
  primal.b->add(-1., *primal.Fu);
  slab->spacetime.primal.constraints->distribute(*primal.b);
}

template <int dim>
void Fluid<dim>::primal_calculate_boundary_values(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  std::shared_ptr<dealii::VectorFunctionFromTensorFunction<dim>>
      dirichlet_function =
          std::make_shared<dealii::VectorFunctionFromTensorFunction<dim>>(
              *function.convection.dirichlet, 0, (dim + 1));
  ...
  for (auto colour : boundary_colours) {
    ...
    auto component_mask_convection =
        std::make_shared<dealii::ComponentMask>((dim + 1), false);
    ...
    // std::map<dealii::types::global_dof_index, double>
    {
      ...
      if (!parameter_set->fe.primal_order.compare("low")) {
        if (!(parameter_set->fe.low.convection.time_type_support_points.compare(
                "Gauss"))) {
          support_points = std::make_shared<dealii::QGauss<1>>(
              (parameter_set->fe.low.convection.r + 1));
        } else if (!(parameter_set->fe.low.convection.time_type_support_points
                         .compare("Gauss-Lobatto"))) {
          if (parameter_set->fe.low.convection.r < 1) {
            support_points = std::make_shared<QRightBox<1>>();
          } else {
            support_points = std::make_shared<dealii::QGaussLobatto<1>>(
                (parameter_set->fe.low.convection.r + 1));
          }
        }
      } else {
        if (!(parameter_set->fe.high.convection.time_type_support_points
                  .compare("Gauss"))) {
          support_points = std::make_shared<dealii::QGauss<1>>(
              (parameter_set->fe.high.convection.r + 1));

        } else if (!(parameter_set->fe.high.convection.time_type_support_points
                         .compare("Gauss-Lobatto"))) {
      ...
      dealii::FEValues<1> time_fe_values(
          *slab->time.primal.fe_info->mapping, *slab->time.primal.fe_info->fe,
          *support_points, dealii::update_quadrature_points);

      for (; cell_time != endc_time; ++cell_time) {
        time_fe_values.reinit(cell_time);

        for (unsigned int qt{0}; qt < support_points->size(); ++qt) {
          dirichlet_function->set_time(time_fe_values.quadrature_point(qt)[0]);
          ...
          std::map<dealii::types::global_dof_index, double> boundary_values_qt;

          if (zero) {
            dealii::VectorTools::interpolate_boundary_values(
                *slab->space.primal.fe_info->dof,
                static_cast<dealii::types::boundary_id>(colour),
                dealii::ZeroFunction<dim>(dim + 1), boundary_values_qt,
                *component_mask_convection);
          } else {
            dealii::VectorTools::interpolate_boundary_values(
                *slab->space.primal.fe_info->dof,
                static_cast<dealii::types::boundary_id>(colour),
                *dirichlet_function, boundary_values_qt,
                *component_mask_convection);
          }
...
template <int dim>
void Fluid<dim>::primal_apply_bc(
    std::map<dealii::types::global_dof_index, double> &boundary_values,
...
void Fluid<dim>::primal_apply_bc(
  ...
  if (boundary_values.size()) {
    ...
    // internal checks
    Assert(A->m(),
           dealii::ExcMessage("empty operator matrix A (rows: A.m() == 0)"));

    Assert(A->n(),
           dealii::ExcMessage("empty operator matrix A (cols: A.n() == 0)"));

    Assert((A->n() == x->size()),
           dealii::ExcMessage("Internal dimensions error: "
    ...
    for (auto &boundary_value : boundary_values) {
      ...
      Assert((boundary_value.first < A->m()),
             dealii::ExcMessage("constraining of dof index (as boundary value) "
    ...
    // 			//
    // 			// NOTE: this is quite expensive, but helps iterative
    // lss
    // 			//       since the boundary value entries are shifted to
    // the
    // 			//       right hand side.
    // 			//
    // 			// NOTE: there is no symmetry assumption on the sparsity
    // pattern,
    // 			//       which is necessary for space-time operators
    // 			//
    // 			for (dealii::types::global_dof_index i{0}; i < A->m(); ++i)
    // {
    // 				// if the row i of the operator A is not
    // constrained,
    // 				// check if constrained columns need to be
    // eliminated 				if (boundary_values.find(i) == boundary_values.end()) {
...
template <int dim>
void Fluid<dim>::primal_solve_slab_problem(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  primal.b->reinit(*slab->spacetime.primal.locally_owned_dofs, mpi_comm);

  primal.du->reinit(*slab->spacetime.primal.locally_owned_dofs, mpi_comm);

  primal.owned_tmp->reinit(*slab->spacetime.primal.locally_owned_dofs,
                           mpi_comm);

  primal.relevant_tmp->reinit(
      *slab->space.primal.fe_info->locally_owned_dofs,
      *slab->space.primal.fe_info->locally_relevant_dofs, mpi_comm);
  ...
  DTM::pout
      << "dwr-instatfluid: apply previous solution as initial Newton guess...";
  ...
  for (; lri != lre; lri++) {
    for (unsigned int ii{0}; ii < slab->time.primal.fe_info->dof->n_dofs();
         ii++) {
      (*primal
            .owned_tmp)[*lri + slab->space.primal.fe_info->dof->n_dofs() * ii] =
          (*primal.um)[*lri];
    }
  }
  ...
  *primal.relevant_tmp = *primal.um;
  ...
  while (newton_residual > newton.lower_bound &&
         newton_step <= newton.max_steps) {
    ...
    if (newton_residual / old_newton_residual > newton.rebuild) {
      // NOTE: for Stokes without adaptive refinement the system matrix needs
      // only to be inverted on the first slab
      if (!parameter_set->problem.compare("Navier-Stokes") ||
          !parameter_set->dwr.refine_and_coarsen.spacetime.strategy.compare(
              "adaptive") ||
          slab->t_m == parameter_set->time.fluid.t0) {
        primal_assemble_system(slab, u->x[0]);

        primal.iA = nullptr;
        //
        primal.iA = std::make_shared<dealii::TrilinosWrappers::SolverDirect>(
            *primal.sc, *primal.ad);
    ...
    slab->spacetime.primal.constraints->distribute(*primal.du);
    ...
    *primal.owned_tmp = *u->x[0];
    for (line_search_step = 0; line_search_step < newton.line_search_steps;
      ...
      *u->x[0] = *primal.owned_tmp;
      //			DTM::pout << "u at ls step " << line_search_step
      //<< std::endl; 		    u->x[0]->print(DTM::pout); 			u->x[0]->add(1.0,*primal.du);
    ...
    *primal.owned_tmp = *u->x[0];
    DTM::pout << std::setprecision(5) << newton_step << "\t" << std::scientific
    ...
    if (line_search_step == newton.line_search_steps &&
        std::abs(newton_residual - old_newton_residual) < 1e-15) {
      break;
    }
    newton_step++;
  }
...
template <int dim>
void Fluid<dim>::primal_do_forward_TMS(const unsigned int dwr_loop, bool last) {
  ////////////////////////////////////////////////////////////////////////////
  ...
  while (slab != grid->slabs.end()) {
    DTM::pout << "primal: solving problem on "
              << "Q_" << n << " = Omega_h x (" << slab->t_m << ", " << slab->t_n
              << ") " << std::endl;
...

    {
      ...
      idealii::SlabDoFTools::make_spacetime_constraints(
          slab->space.primal.fe_info->locally_relevant_dofs,
          slab->space.primal.fe_info
              ->hanging_node_constraints,  // space constraints
          slab->space.primal.fe_info->dof->n_dofs(),
          slab->time.primal.fe_info->dof->n_dofs(),
          slab->spacetime.primal.locally_relevant_dofs,
          slab->spacetime.primal.hanging_node_constraints);
    ...
    if (!parameter_set->problem.compare("Navier-Stokes") ||
      ...
      grid->create_sparsity_pattern_primal_on_slab(slab, primal.L,
                                                   primal.projection_matrix);
    }
    ...
    primal.um->reinit(*slab->space.primal.fe_info->locally_owned_dofs,
                      mpi_comm);
    *primal.um = 0.;
    if (slab == grid->slabs.begin()) {
      ...
      //			Assert(function.u_0.use_count(),
      //dealii::ExcNotInitialized()); 			function.u_0->set_time(slab->t_m);
      ...
      dealii::VectorTools::interpolate(*slab->space.primal.fe_info->mapping,
                                       *slab->space.primal.fe_info->dof,
      ...
      //			primal.um_projected = std::make_shared<
      //dealii::TrilinosWrappers::MPI::Vector > ();
      ...
      *un_rel = *primal.un;
      ...
      dealii::VectorTools::interpolate_to_different_mesh(
          // solution on I_{n-1}:
          *std::prev(slab)->space.primal.fe_info->dof, *un_rel,
          // solution on I_n:
          *slab->space.primal.fe_info->dof,
          *slab->space.primal.fe_info->hanging_node_constraints, *primal.um);
    }
    ...
    *primal.un = 0.;

    {
      dealii::FEValues<1> fe_face_values_time(
          *slab->time.primal.fe_info->mapping, *slab->time.primal.fe_info->fe,
          dealii::QGaussLobatto<1>(2), dealii::update_values);
    ...
    if (parameter_set->primal_only) {
      // also clear primal solution, since it is not being needed for error
      // estimation anymore
      u->x[0]->clear();
      um->x[0]->clear();
    }
  ...
  if (parameter_set->dwr.functional.mean_drag) {
    reference_goal_functional =
        error_estimator.goal_functional.reference.mean_drag;
    fem_goal_functional = error_estimator.goal_functional.fem.mean_drag;

    DTM::pout << "---------------------------" << std::endl;
    DTM::pout << "Mean drag:" << std::endl;
    DTM::pout << "	J(u)               = " << std::setprecision(16)
              << reference_goal_functional << std::endl;
    DTM::pout << "	J(u_{kh})          = " << std::setprecision(16)
              << fem_goal_functional << std::endl;
    DTM::pout << "	|J(u) - J(u_{kh})| = " << std::setprecision(16)
              << std::abs(reference_goal_functional - fem_goal_functional)
              << std::endl;
    DTM::pout << "---------------------------" << std::endl << std::endl;
  }
...
template <int dim>
void Fluid<dim>::primal_init_data_output() {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  DTM::pout << "primal solution data output: patches = "
            << parameter_set->data_output.primal.patches << std::endl;
  ...
  primal.data_postprocessor =
      std::make_shared<fluid::DataPostprocessor<dim>>(output_quantities);
  ...
  if (primal.data_output_trigger_type_fixed) {
    DTM::pout << "primal solution data output: using fixed mode with trigger = "
              << primal.data_output_trigger << std::endl;
  } else {
    DTM::pout << "primal solution data output: using I_n mode (trigger adapts "
                 "to I_n automatically)"
              << std::endl;
  }
...
template <int dim>
void Fluid<dim>::primal_do_data_output_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &u,
  ...
  filename << "solution-dwr_loop-" << std::setw(setw_value_dwr_loops)
           << std::setfill('0') << dwr_loop;
  //
  //	{
  ...
  //			// if triggered output time values are inside this time
  //element
  //			//
  //
  ...
  //					(std::abs(primal.data_output_time_value - t_n) <
  //tau*1e-12)
  //				); ) {
  //				output_times.push_back(primal.data_output_time_value);
  //				primal.data_output_time_value +=
  ...
  //output_time_points(output_times.size());
  //			{
  //				auto time{output_times.begin()};
  //				for (unsigned int q{0}; q < output_time_points.size();
  //++q,++time) { 					double t_trigger{*time}; 					output_time_points[q][0] =
  //(t_trigger-t_m)/tau;
  //				}
  //
  ...
  //local_dof_indices(slab->time.primal.fe_info->fe->dofs_per_cell);
  //			cell_time->get_dof_indices(local_dof_indices);
  //
  //			for (unsigned int qt{0}; qt <
  //fe_values_time.n_quadrature_points; ++qt) {
  // 				*u_trigger = 0.;
  //
  ...
  // 						// time offset
  // 						+ slab->space.primal.fe_info->dof->n_dofs()
  // * 							local_dof_indices[jj] 					] * fe_values_time.shape_value(jj,qt);
  // 				}}
  //
  ////				std::cout
...
template <int dim>
void Fluid<dim>::primal_do_data_output_on_slab_Qn_mode(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &u,
  ...
  primal.data_output->set_DoF_data(slab->space.primal.fe_info->dof);
  ...
  if (parameter_set->dwr.functional.mean_vorticity) {
    vort_trigger->reinit(*slab->space.vorticity.fe_info->locally_owned_dofs,
                         *slab->space.vorticity.fe_info->locally_relevant_dofs,
                         mpi_comm);

    vort_tmp->reinit(*slab->space.vorticity.fe_info->locally_owned_dofs,
                     mpi_comm);
  }
  ...
  vortname << "vorticity-dwr_loop-" << std::setw(setw_value_dwr_loops)
           << std::setfill('0') << dwr_loop;

  {
    // create fe values
    std::shared_ptr<dealii::Quadrature<1>> fe_quad_time;
    if (!(parameter_set->fe.primal.convection.time_type_support_points.compare(
            "Gauss"))) {
      fe_quad_time = std::make_shared<dealii::QGauss<1>>(
          (parameter_set->fe.primal.convection.r + 1));

    } else if (!(parameter_set->fe.primal.convection.time_type_support_points
                     .compare("Gauss-Lobatto"))) {
      if (parameter_set->fe.primal.convection.r < 1) {
        fe_quad_time = std::make_shared<QRightBox<1>>();
      } else {
        fe_quad_time = std::make_shared<dealii::QGaussLobatto<1>>(
            (parameter_set->fe.primal.convection.r + 1));
      }
    }

    AssertThrow(fe_quad_time.use_count(),
                dealii::ExcMessage(
        ...
        *u_trigger = *owned_tmp;
        ...
        if (parameter_set->dwr.functional.mean_vorticity) {
          ...
          *vort_tmp = 0.;
          ...
          dealii::IndexSet::ElementIterator lri =
              slab->space.vorticity.fe_info->locally_owned_dofs->begin();
          dealii::IndexSet::ElementIterator lre =
              slab->space.vorticity.fe_info->locally_owned_dofs->end();
...
template <int dim>
void Fluid<dim>::primal_do_data_output(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &x,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &vort,
    const unsigned int dwr_loop, bool last) {
  ...
  if (primal.data_output_dwr_loop < 0) return;
...
template <int dim>
void Fluid<dim>::dual_reinit_storage() {
  ////////////////////////////////////////////////////////////////////////////
  // init storage containers for vector data:
  ...
  dual.storage.z->resize(static_cast<unsigned int>(grid->slabs.size()));
}

template <int dim>
void Fluid<dim>::dual_reinit_storage_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
  ...
  for (unsigned int j{0}; j < z->x.size(); ++j) {
    ...
    Assert(slab->time.dual.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());

    z->x[j]->reinit(*slab->spacetime.dual.locally_owned_dofs,
                    *slab->spacetime.dual.locally_relevant_dofs, mpi_comm);
  }
}

template <int dim>
void Fluid<dim>::dual_assemble_system(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> u) {
  // ASSEMBLY SPACE-TIME OPERATOR MATRIX /////////////////////////////////////
  ...
  *dual.L = 0;
  ...
  {
    fluid::spacetime::dual::Operator::Assembler<dim> dual_assembler;

    dual_assembler.set_symmetric_stress(parameter_set->fe.symmetric_stress);

    Assert(function.viscosity.use_count(), dealii::ExcNotInitialized());
    dual_assembler.set_functions(function.viscosity);
    ...
    DTM::pout
        << "dynamic fluid: assemble space-time slab dual operator matrix...";
...
template <int dim>
void Fluid<dim>::dual_assemble_rhs(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> u) {
  // NOTE: for nonlinear goal functionals, we need also the primal solution u
  // for this function
  ...
  dual.Mzn->reinit(*slab->spacetime.dual.locally_owned_dofs, mpi_comm);
  *dual.Mzn = 0.;
  ...
  *dual.relevant_tmp = *dual.zn;
  {
    ...
    DTM::pout
        << "dwr-instatfluid: assemble space-time slab final value vector...";
    Assert(dual.zn.use_count(), dealii::ExcNotInitialized());
    Assert(dual.Mzn.use_count(), dealii::ExcNotInitialized());
    dual_assembler.assemble(dual.relevant_tmp, dual.Mzn, slab);
  ...
  dual.Je->reinit(*slab->spacetime.dual.locally_owned_dofs, mpi_comm);
  *dual.Je = 0.;
  ...
  if (!parameter_set->dwr.goal.type.compare("mean_drag")) {
    ...
    Assert(function.viscosity.use_count(), dealii::ExcNotInitialized());
    Je_assembler.set_functions(function.viscosity);

    DTM::pout
        << "dwr-instatfluid: assemble space-time slab Je_meandrag vector...";
    Je_assembler.assemble(dual.Je, slab, parameter_set->time.fluid.t0,
                          parameter_set->time.fluid.T);

    DTM::pout << " (done)" << std::endl;
  } else if (!parameter_set->dwr.goal.type.compare("mean_vorticity")) {
    Je_MeanVorticity_Assembler<dim> Je_assembler;
    DTM::pout << "dwr-instatfluid: assemble space-time slab Je_meanviscosity "
                 "vector...";
    Je_assembler.assemble(dual.Je, slab, u, parameter_set->time.fluid.t0,
                          parameter_set->time.fluid.T);
...
template <int dim>
void Fluid<dim>::dual_solve_slab_problem(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &z) {
  Assert(dual.L.use_count(), dealii::ExcNotInitialized());
  Assert(dual.Mzn.use_count(), dealii::ExcNotInitialized());
  ...
  dual.b->reinit(*slab->spacetime.dual.locally_owned_dofs, mpi_comm);
  ...
  *dual.b = 0.;
  ...
  if (!parameter_set->problem.compare("Navier-Stokes") ||
    ...
    dual.iA = nullptr;
    dual.iA = std::make_shared<dealii::TrilinosWrappers::SolverDirect>(
        *dual.sc, *dual.ad);
  ...
  slab->spacetime.dual.constraints->distribute(*dual.owned_tmp);

  *z->x[0] = *dual.owned_tmp;
}

template <int dim>
void Fluid<dim>::dual_do_backward_TMS(const unsigned int dwr_loop, bool last) {
  ////////////////////////////////////////////////////////////////////////////
  // prepare time marching scheme (TMS) loop
  //
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  // error indicators
  Assert(error_estimator.storage.eta_space.use_count(),
         dealii::ExcNotInitialized());
  Assert(error_estimator.storage.eta_space->size(),
         dealii::ExcNotInitialized());
  auto eta_space = std::prev(error_estimator.storage.eta_space->end());

  Assert(error_estimator.storage.eta_time.use_count(),
         dealii::ExcNotInitialized());
  ...
  while (n) {
    DTM::pout << "dual: solving problem on "
              << "Q_" << n << " = Omega_h x (" << slab->t_m << ", " << slab->t_n
              << ") " << std::endl;
    ...
    if (parameter_set->fe.primal_order.compare(parameter_set->fe.dual_order)) {
      if (!parameter_set->fe.dual_order.compare("low")) {
        ...
        grid->distribute_low_on_slab(slab);
                 ...
                 !parameter_set->fe.dual_order.compare("high-time")) {
        grid->initialize_high_grid_components_on_slab(slab);
        grid->distribute_high_on_slab(slab);
      } else
        AssertThrow(false, dealii::ExcNotImplemented());
    } else  // (primal_order == dual_order): reinit the remaining order (high or
            // low), since it is needed for error estimation
    {
      if (!parameter_set->fe.dual_order.compare("low")) {
    ...
    // dual dof partitioning
    {
      slab->spacetime.dual.locally_owned_dofs =
          std::make_shared<dealii::IndexSet>(
              idealii::SlabDoFTools::extract_locally_owned_dofs(
                  slab->space.dual.fe_info->dof, slab->time.dual.fe_info->dof));
    ...
    if (!parameter_set->problem.compare("Navier-Stokes") ||
        !parameter_set->dwr.refine_and_coarsen.spacetime.strategy.compare(
            "adaptive") ||
        std::abs(slab->t_n - parameter_set->time.fluid.T) <
            0.5 * slab->tau_n()) {
      dual.iA = std::make_shared<dealii::TrilinosWrappers::SolverDirect>(
          *dual.sc, *dual.ad);
...

    {
      ...
      // pu dof partitioning
      {
        slab->spacetime.pu.locally_owned_dofs =
            std::make_shared<dealii::IndexSet>(
                idealii::SlabDoFTools::extract_locally_owned_dofs(
                    slab->space.pu.fe_info->dof, slab->time.pu.fe_info->dof));
      ...
      eta_reinit_storage_on_slab(slab, eta_space, eta_time);

      error_estimator.pu_dwr =
          std::make_shared<fluid::cGp_dGr::cGq_dGs::ErrorEstimator<dim>>();
      // set the important variables for the error estimator
      error_estimator.pu_dwr->init(
          mpi_comm, function.viscosity, grid,
          parameter_set->fe.symmetric_stress,
          parameter_set->dwr.replace_linearization_points,
          parameter_set->dwr.replace_weights, parameter_set->fe.primal_order,
    ...
    *dual.zn = 0.;
    ...
    if (n == N) {
      ...
      dealii::VectorTools::interpolate(
          *slab->space.dual.fe_info->mapping, *slab->space.dual.fe_info->dof,
          dealii::ZeroFunction<dim>(dim + 1), *dual.zn);

      // NOTE: after the first dwr-loop the initial triangulation could have
      //       hanging nodes. Therefore,
      // distribute hanging node constraints to make the result continuous
      // again:
      slab->space.dual.fe_info->constraints->distribute(*dual.zn);
    } else {
      ...
      zm_rel->reinit(
          *std::next(slab)->space.dual.fe_info->locally_owned_dofs,
      ...
      *zm_rel = *dual.zm;
      ...
      dealii::VectorTools::interpolate_to_different_mesh(
          // solution on I_{n+1}:
          *std::next(slab)->space.dual.fe_info->dof, *zm_rel,
          // solution on I_n:
          *slab->space.dual.fe_info->dof,
          *slab->space.dual.fe_info->constraints, *dual.zn);
    ...
    // NOTE: for Stokes without adaptive refinement the dual system matrix needs
    // only to be inverted on the last slab
    if (!parameter_set->problem.compare("Navier-Stokes") ||
        !parameter_set->dwr.refine_and_coarsen.spacetime.strategy.compare(
            "adaptive") ||
        std::abs(slab->t_n - parameter_set->time.fluid.T) <
    ...
    // to extrapolate in time, we also need z(t_m^-) which is from the solution
    // from the last time slab hence we always lag one slab with the error
    // estimation
    //
    Assert(error_estimator.pu_dwr.use_count(), dealii::ExcNotInitialized());
    // evaluate error on next slab
    if (n < N) {
      error_estimator.pu_dwr->estimate_on_slab(
          std::next(slab), std::next(u), std::next(um), std::next(z),
          std::next(eta_space), std::next(eta_time));
      std::next(u)->x[0]->clear();
      std::next(um)->x[0]->clear();
      std::next(slab)->space.primal.fe_info->dof->clear();

      // TODO: some Segfault here, no idea why
      //			// apply B. Endtmayer's post processing of the
      //error indicators
      //			// see https://arxiv.org/pdf/1811.07586.pdf
      //(Figure 1) 			auto space_relevant = std::make_shared<
      //dealii::TrilinosWrappers::MPI::Vector > (); 			auto time_relevant =
      //std::make_shared< dealii::TrilinosWrappers::MPI::Vector > ();
      //
      ...
      //
      //			for (auto line :
      //std::next(slab)->spacetime.pu.constraints->get_lines())
      //			{
      //				//go over all line entries
      //				for (unsigned int i=0; i<std::pow(2,
      //dim-1); ++i){ 					if
      //(std::next(slab)->spacetime.pu.locally_owned_dofs->is_element(line.entries[i].first)){
      //						(*std::next(eta_space)->x[0])[line.entries[i].first]
      //						  += (1. / std::pow(2,
      //dim-1))*(*space_relevant)[line.index];
      //
      //						(*std::next(eta_time)->x[0])[line.entries[i].first]
      //						  += (1. / std::pow(2,
      //dim-1))*(*time_relevant)[line.index];
      //
      //					}
      //				}
      //				if
      //(std::next(slab)->spacetime.pu.locally_owned_dofs->is_element(line.index)){
      //					(*std::next(eta_space)->x[0])[line.index]
      //= 0.;
      //					(*std::next(eta_time)->x[0])[line.index]
      //= 0.;
      //				}
      //			}
    }
    ...
    if (n == 1) {
      error_estimator.pu_dwr->estimate_on_slab(slab, u, um, z, eta_space,
                                               eta_time);
      u->x[0]->clear();
      um->x[0]->clear();
      ...
      //std::make_shared< dealii::TrilinosWrappers::MPI::Vector > ();
      //
      //			std::cout << "reiniting relevant vectors" <<
      //std::endl; 			space_relevant ->reinit(
      //					*slab->spacetime.pu.locally_owned_dofs,
      //					*slab->spacetime.pu.locally_relevant_dofs,
      //					mpi_comm
      //			);
      ...
      //			);
      //
      //			std::cout << "communicating estimators" <<
      //std::endl; 			*space_relevant = *eta_space->x[0]; 			*time_relevant =
      //*eta_time->x[0];
      //
      //			std::cout << "Starting B.E. postprocessing" <<
      //std::endl; 			for (auto line : slab->spacetime.pu.constraints->get_lines())
      //			{
      ...
      //				//go over all line entries
      //				for (unsigned int i=0; i<std::pow(2,
      //dim-1); ++i){ 					if (slab->spacetime.pu.locally_owned_dofs->is_element(
      ...
      //					}
      //				}
      //				std::cout << "entries done, zeroing
      //index on rank "
      //						  <<
      //dealii::Utilities::MPI::this_mpi_process(mpi_comm)
      //						  << std::endl;
      //				if
      //(slab->spacetime.pu.locally_owned_dofs->is_element(line.index)){
      //					(*eta_space->x[0])[line.index] =
      //0.;
      //					(*eta_time->x[0])[line.index] =
      //0.;
      //				}
      //			}
    }
    ...
    *dual.zm = 0.;
    ...
    {
      dealii::FEValues<1> fe_face_values_time(
          *slab->time.dual.fe_info->mapping, *slab->time.dual.fe_info->fe,
          dealii::QGaussLobatto<1>(2), dealii::update_values);
      ...
      cell_time = last_cell_time;
      {
        ...
        for (unsigned int jj{0};
          ...
          dealii::IndexSet::ElementIterator lri =
              slab->space.dual.fe_info->locally_owned_dofs->begin();
          dealii::IndexSet::ElementIterator lre =
              slab->space.dual.fe_info->locally_owned_dofs->end();
    ...
    if (n < N)
      eta_do_data_output(std::next(slab), std::next(eta_space),
                         std::next(eta_time), dwr_loop, last);
    if (n == 1) eta_do_data_output(slab, eta_space, eta_time, dwr_loop, last);
...
template <int dim>
void Fluid<dim>::dual_init_data_output() {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  // may output data: initialise (mode: all, last or specific dwr loop)
  DTM::pout << "dual solution data output: patches = "
            << parameter_set->data_output.dual.patches << std::endl;
  ...
  if (dual.data_output_trigger_type_fixed) {
    DTM::pout << "dual solution data output: using fixed mode with trigger = "
              << dual.data_output_trigger << std::endl;
  } else {
    DTM::pout << "dual solution data output: using I_n mode (trigger adapts to "
                 "I_n automatically)"
              << std::endl;
  }
...
template <int dim>
void Fluid<dim>::dual_do_data_output_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &z,
    const unsigned int dwr_loop) {
  // TODO: might need to be debugged; adapted from
  ...
  // > (); 	z_trigger->reinit( 		slab->space.dual.fe_info->dof->n_dofs()
  // 	);
  //
  std::ostringstream filename;
  filename << "dual-dwr_loop-" << std::setw(setw_value_dwr_loops)
           << std::setfill('0') << dwr_loop;
  //
  //	{
  //		// fe face values time: time face (I_n) information
  ...
  //					(std::abs(dual.data_output_time_value - t_n) <
  //tau*1e-12)
  //				); ) {
  //				output_times.push_back(dual.data_output_time_value);
  //				dual.data_output_time_value +=
  ...
  //
  //			std::vector< dealii::types::global_dof_index >
  //local_dof_indices(slab->time.dual.fe_info->fe->dofs_per_cell);
  //			cell_time->get_dof_indices(local_dof_indices);
  //
  //			for (unsigned int qt{0}; qt <
  //fe_values_time.n_quadrature_points; ++qt) {
...
template <int dim>
void Fluid<dim>::dual_do_data_output_on_slab_Qn_mode(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &z,
    const unsigned int dwr_loop) {
  // natural output of solutions on Q_n in their support points in time
  Assert(slab->space.dual.fe_info->dof.use_count(),
         dealii::ExcNotInitialized());
  dual.data_output->set_DoF_data(slab->space.dual.fe_info->dof);

  auto z_trigger = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  z_trigger->reinit(*slab->space.dual.fe_info->locally_owned_dofs,
                    *slab->space.dual.fe_info->locally_relevant_dofs, mpi_comm);

  auto owned_tmp = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  owned_tmp->reinit(*slab->space.dual.fe_info->locally_owned_dofs, mpi_comm);
...

  {
    // create fe values
    std::shared_ptr<dealii::Quadrature<1>> fe_quad_time;
    if (!(parameter_set->fe.dual.convection.time_type_support_points.compare(
            "Gauss"))) {
      fe_quad_time = std::make_shared<dealii::QGauss<1>>(
          (parameter_set->fe.primal.convection.r + 1));

    } else if (!(parameter_set->fe.dual.convection.time_type_support_points
                     .compare("Gauss-Lobatto"))) {
    ...
    dealii::FEValues<1> fe_values_time(
        *slab->time.dual.fe_info->mapping, *slab->time.dual.fe_info->fe,
        dealii::QGauss<1>(slab->time.dual.fe_info->fe->tensor_degree() +
                          1),  // here
        dealii::update_values | dealii::update_quadrature_points);
    ...
    for (; cell_time != endc_time; ++cell_time) {
      ...
      for (unsigned int qt{0}; qt < fe_values_time.n_quadrature_points; ++qt) {
        ...
        *z_trigger = *owned_tmp;
        ...
        dual.data_output->write_data(
            filename.str(), z_trigger, dual.data_postprocessor,
            fe_values_time.quadrature_point(qt)[0]  // t_trigger
        );
      }
    }
...
template <int dim>
void Fluid<dim>::dual_do_data_output(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &z,
    const unsigned int dwr_loop, bool last) {
  ...
  if (dual.data_output_dwr_loop < 0) return;

  if (static_cast<unsigned int>(dual.data_output_dwr_loop) != dwr_loop) return;
...
template <int dim>
void Fluid<dim>::dual_sort_xdmf_by_time(const unsigned int dwr_loop) {
  if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0) {
    // name of xdmf file to be sorted by time of the snapshots
    std::ostringstream filename;
    filename << "dual-dwr_loop-" << std::setw(setw_value_dwr_loops)
             << std::setfill('0') << dwr_loop << ".xdmf";
    ...
    // create a std::vector for the new lines where the grids are sorted in
    // ascending order by the time
    std::vector<std::string> new_lines;

    // the first 5 lines remain unchanged
    for (unsigned int i = 0; i <= 4; ++i) new_lines.push_back(old_lines[i]);
    ...
    std::sort(std::begin(grids), std::end(grids),
    ...
    for (auto line : new_lines) new_xdmf_file << line << std::endl;
    new_xdmf_file.close();
  }
}
...
//
template <int dim>
void Fluid<dim>::compute_functional_values(
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &u,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &vort,
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ...
  dealii::FEValues<1> fe_values_time(
      *slab->time.primal.fe_info->mapping, *slab->time.primal.fe_info->fe,
      *quad_time, dealii::update_quadrature_points | dealii::update_JxW_values);
  ...
  for (; cell_time != endc_time; ++cell_time) {
    ...
    for (unsigned int ii{0}; ii < slab->time.primal.fe_info->fe->dofs_per_cell;
         ++ii) {
      un->reinit(*slab->space.primal.fe_info->locally_owned_dofs, mpi_comm);
      ...
      *un_rel = *un;
      ...
      double scaling =
          1. / (parameter_set->time.fluid.T - parameter_set->time.fluid.t0);
      ...
      if (parameter_set->dwr.functional.mean_pdiff) {
        ...
        double pressure_front =
            compute_pressure(M, un_rel,
                             slab);  // pressure - left  point on circle
        ...
        double pressure_back =
            compute_pressure(M, un_rel,
                             slab);  // pressure - right point on circle
        ...
        error_estimator.goal_functional.fem.mean_pdiff +=
            scaling * pressure_diff * fe_values_time.JxW(ii);
      }
      ////////////////////////////////////
      // drag and lift
      if (parameter_set->dwr.functional.mean_drag ||
          parameter_set->dwr.functional.mean_lift) {
        // Compute drag and lift via line integral
        dealii::Tensor<1, dim> drag_lift_value;
        compute_drag_lift_tensor(un_rel, slab, drag_lift_value);
        if (parameter_set->dwr.functional.mean_drag) {
          drag_values.push_back(std::make_tuple(tn, drag_lift_value[0]));
          error_estimator.goal_functional.fem.mean_drag +=
              scaling * drag_lift_value[0] * fe_values_time.JxW(ii);
        }
        if (parameter_set->dwr.functional.mean_lift) {
          lift_values.push_back(std::make_tuple(tn, drag_lift_value[1]));
          error_estimator.goal_functional.fem.mean_lift +=
              scaling * drag_lift_value[1] * fe_values_time.JxW(ii);
        }
      }
      ...
      if (parameter_set->dwr.functional.mean_vorticity) {
        ...
        *vortn_rel = *vortn;
        ...
        error_estimator.goal_functional.fem.mean_vorticity +=
            scaling * mv2 * fe_values_time.JxW(ii);
        ...
        for (; lri != lre; lri++) {
          (*vort_owned)[*lri
                        ...
                        + slab->space.vorticity.fe_info->dof->n_dofs() *
                              (cell_time->index() *
                               slab->time.vorticity.fe_info->fe->dofs_per_cell)
                        // local in time dof
  ...
  if (parameter_set->dwr.functional.mean_pdiff) {
    ...
    for (auto &item : pressure_values) {
      DTM::pout << "	" << std::setw(14) << std::setprecision(8)
                << std::get<0>(item);
      DTM::pout << ":    " << std::setprecision(16) << std::get<1>(item)
                << std::endl;
...
template <int dim>
double Fluid<dim>::compute_pressure(
    dealii::Point<dim> x,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> un,
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab) {
  ...
  try {
    dealii::VectorTools::point_value(*slab->space.primal.fe_info->dof,
...
template <int dim>
void Fluid<dim>::compute_drag_lift_tensor(
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> un,
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    dealii::Tensor<1, dim> &drag_lift_value) {
  const dealii::QGauss<dim - 1> face_quadrature_formula(
      std::max(std::max(slab->space.primal.fe_info->fe->base_element(0)
                            .base_element(0)
                            .tensor_degree(),
                        slab->space.primal.fe_info->fe->base_element(0)
                            .base_element(1)
                            .tensor_degree()),
               static_cast<unsigned int>(1)) +
      4);
  dealii::FEFaceValues<dim> fe_face_values(
      *slab->space.primal.fe_info->fe, face_quadrature_formula,
      dealii::update_values | dealii::update_gradients |
          dealii::update_normal_vectors | dealii::update_JxW_values |
          dealii::update_quadrature_points);

  const unsigned int dofs_per_cell =
      slab->space.primal.fe_info->fe->dofs_per_cell;
  const unsigned int n_face_q_points = face_quadrature_formula.size();

  std::vector<unsigned int> local_dof_indices(dofs_per_cell);
  std::vector<dealii::Vector<double>> face_solution_values(
      n_face_q_points, dealii::Vector<double>(dim + 1));

  std::vector<std::vector<dealii::Tensor<1, dim>>> face_solution_grads(
      n_face_q_points, std::vector<dealii::Tensor<1, dim>>(dim + 1));
      ...
      for (unsigned int face = 0;
        ...
        if (cell->face(face)->at_boundary() &&
          ...
          for (unsigned int q = 0; q < n_face_q_points; ++q) {
            ...
            drag_lift_value -= sigma_fluid * fe_face_values.normal_vector(q) *
                               fe_face_values.JxW(q);
          }
        }  // end boundary
           // stokes::types::space::boundary_id::prescribed_obstacle for fluid
    }      // end cell
  ...
  if (dim == 2) {
    max_velocity = function.convection.dirichlet->value(dealii::Point<dim>(
        0.0, 0.205)  // maximal velocity in the middle of the boundary
                                                        )[0];
  }

  if (parameter_set->convection.dirichlet_boundary_function.compare(
          "Convection_Parabolic_Inflow_3") == 0) {
    if (max_velocity < 0.3 + 1e-9)
      drag_lift_value *= 500.0;  // 2D-1
    else
      drag_lift_value *= 20.0;  // 2D-2
  } else if (parameter_set->convection.dirichlet_boundary_function.compare(
                 "Convection_Parabolic_Inflow_3_sin") == 0) {
    if (dim == 2)
...
template <int dim>
double Fluid<dim>::compute_vorticity(
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> un,
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> vortn) {
  ...
  dealii::FEValues<dim> fe_values_primal(
      *slab->space.primal.fe_info->fe, quadrature_formula,
      dealii::update_values | dealii::update_gradients |
          dealii::update_JxW_values | dealii::update_quadrature_points);

  dealii::FEValues<dim> fe_values_vort(
      *slab->space.vorticity.fe_info->fe, quadrature_formula,
      dealii::update_values | dealii::update_quadrature_points |
          dealii::update_JxW_values);
  dealii::IndexSet loe = vortn->locally_owned_elements();

  const unsigned int dofs_per_cell =
      slab->space.vorticity.fe_info->fe->dofs_per_cell;
...
template <int dim>
void Fluid<dim>::eta_reinit_storage() {
  ////////////////////////////////////////////////////////////////////////////
  // init storage containers for vector data:
...
template <int dim>
void Fluid<dim>::eta_reinit_storage_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator
        &eta_s,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator
        &eta_t) {
  ...
  for (unsigned int j{0}; j < eta_s->x.size(); ++j) {
    ...
    Assert(slab->space.pu.fe_info->dof.use_count(),
           dealii::ExcNotInitialized());

    Assert(slab->time.pu.fe_info->dof.use_count(), dealii::ExcNotInitialized());

    eta_s->x[j]->reinit(*slab->spacetime.pu.locally_owned_dofs, mpi_comm);
  }
  ...
  for (unsigned int j{0}; j < eta_t->x.size(); ++j) {
    ...
    eta_t->x[j]->reinit(*slab->spacetime.pu.locally_owned_dofs, mpi_comm);
  }
}

template <int dim>
void Fluid<dim>::compute_effectivity_index() {
  ...
  DTM::pout << "\neta_k              = " << value_eta_k
            ...
            << "\nI_eff              = " << I_eff << std::endl;
}

template <int dim>
void Fluid<dim>::refine_and_coarsen_space_time_grid(
    const unsigned int dwr_loop) {
  Assert(error_estimator.storage.eta_space->size() == grid->slabs.size(),
         dealii::ExcInternalError());
  Assert(error_estimator.storage.eta_time->size() == grid->slabs.size(),
         dealii::ExcInternalError());
  ...
  // compute eta_k^n on I_n for n=1..N as well as global estimators
  {
    ...
    for (unsigned n_sl{0}; n_sl < N_slabs;
      ...
      Assert((eta_it_k != error_estimator.storage.eta_time->end()),
             dealii::ExcInternalError());
      Assert((eta_it_h != error_estimator.storage.eta_space->end()),
             dealii::ExcInternalError());
      ...
      eta_h_local->reinit(*slab->space.pu.fe_info->locally_owned_dofs,
                          mpi_comm);
      // get local pu's
      // TODO: if using something else than dG(0) in time for pu this should be
      // changed!

      for (auto &cell : slab->time.tria->active_cell_iterators()) {
        *eta_k_local = 0.;
        *eta_h_local = 0.;
        dealii::IndexSet::ElementIterator loi =
            slab->space.pu.fe_info->locally_owned_dofs->begin();
        dealii::IndexSet::ElementIterator loe =
            slab->space.pu.fe_info->locally_owned_dofs->end();
        for (; loi != loe; loi++) {
          (*eta_k_local)[*loi] =
              (*eta_it_k->x[0])[*loi
                                // time offset
        ...
        eta_h_global += eta_h_K * parameter_set->time.fluid.T / (N_int * k_n);
  ...
  // Per definition eta_k[0] is 0 for the primal problem, so just set it to the
  // next time step
  eta_k[0] = eta_k[1];
  if (eta_k[0] == 0.) DTM::pout << "eta_k[0] = " << eta_k[0] << std::endl;
  ...
  if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0) {
    std::ostringstream filename;
    filename << "slabwise_eta_k-dwr_loop-" << std::setw(setw_value_dwr_loops)
             << std::setfill('0') << dwr_loop << ".log";
    ...
    filename << "slabwise_k_n-dwr_loop-" << std::setw(setw_value_dwr_loops)
             << std::setfill('0') << dwr_loop << ".log";
  ...
  if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0) {
    std::ostringstream filename;
    filename << "intervalwise_eta_k-dwr_loop-"
    ...
    filename << "intervalwise_k_n-dwr_loop-" << std::setw(setw_value_dwr_loops)
             << std::setfill('0') << dwr_loop << ".log";
    ...
    for (auto &slab : grid->slabs) {
      for (auto &cell : slab.time.tria->active_cell_iterators()) {
        ...
        eta_k_out << bbox.get_boundary_points().first << "," << eta_k[n_in]
                  << std::endl;

        eta_k_out << bbox.get_boundary_points().second << "," << eta_k[n_in]
                  << std::endl;

        k_n_out << bbox.get_boundary_points().first << ","
                << bbox.side_length(0) << std::endl;
  ...
  if (std::abs(eta_k_global) * equilibration_factor >= std::abs(eta_h_global)) {
    Assert(((parameter_set->dwr.refine_and_coarsen.time.top_fraction >= 0.) &&
    ...
    if (parameter_set->dwr.refine_and_coarsen.time.top_fraction > 0.) {
      ...
      if (parameter_set->dwr.refine_and_coarsen.time.strategy.compare(
              "fixed_fraction") == 0) {
        double D_goal = std::accumulate(eta_k.begin(), eta_k.end(), 0.) *
                        parameter_set->dwr.refine_and_coarsen.time.top_fraction;
        ...
        unsigned int index_for_mark_time_refinement{
            static_cast<unsigned int>(static_cast<int>(std::ceil(
                static_cast<double>(N_int) *
                parameter_set->dwr.refine_and_coarsen.time.top_fraction)))};

        threshold = eta_sorted[index_for_mark_time_refinement < N_int
                                   ? index_for_mark_time_refinement
                                   : N_int - 1];
      }
        ...
        for (auto &cell : slab->time.tria->active_cell_iterators()) {
          if (eta_k[n_in] >= threshold) {
            ...
            DTM::pout << "Marked interval " << n_in
                      << " for temporal refinement" << std::endl;
  ...
  if (std::abs(eta_k_global) <= equilibration_factor * std::abs(eta_h_global)) {
    ...
    for (unsigned int n{0}; slab != ends; ++slab, ++eta_it, ++n) {
      Assert((eta_it != error_estimator.storage.eta_space->end()),
             dealii::ExcInternalError());
      ...
      if (parameter_set->dwr.refine_and_coarsen.space.top_fraction1 == 1.0) {
        ...
        *eta_relevant = *eta_local;
        ...
        const unsigned int dofs_per_cell_pu =
            slab->space.pu.fe_info->fe->dofs_per_cell;
        std::vector<unsigned int> local_dof_indices(dofs_per_cell_pu);
        unsigned int max_n =
            n_active_cells_on_slab * parameter_set->dwr.refine_and_coarsen.space
                                         .max_growth_factor_n_active_cells;

        typename dealii::DoFHandler<dim>::active_cell_iterator cell{
            slab->space.pu.fe_info->dof->begin_active()},
        ...
        for (unsigned int cell_no{0}; cell != endc; ++cell, ++cell_no) {
          if (cell->is_locally_owned()) {
            ...
            for (unsigned int i = 0; i < dofs_per_cell_pu; i++) {
              indicators[cell_no] +=
                  (*eta_relevant)(local_dof_indices[i]) / dofs_per_cell_pu;
            }
          }
        ...
        if (parameter_set->dwr.refine_and_coarsen.space.strategy.compare(
                "RichterWick") == 0) {
          double threshold =
              eta_local->mean_value() *
              parameter_set->dwr.refine_and_coarsen.space.riwi_alpha;

          dealii::GridRefinement::refine(*slab->space.tria, indicators,
                                         threshold, max_n);
        } else {
          // mark for refinement strategy with fixed fraction
          // (similar but not identical to Hartmann Diploma thesis Ex.
          // Sec. 1.4.2)
          ...
          if (parameter_set->dwr.refine_and_coarsen.space.strategy.compare(
            ...
            dealii::GridRefinement::refine_and_coarsen_fixed_fraction(
                *slab->space.tria, indicators, top_fraction,
                parameter_set->dwr.refine_and_coarsen.space.bottom_fraction,
                max_n);
          } else if (parameter_set->dwr.refine_and_coarsen.space.strategy
                         .compare("fixed_number") == 0) {
            ...
            AssertThrow(false,
                        dealii::ExcMessage("unknown spatial refinement"));
          }
        }

        //				// count which percentage of spatial cells
        //have been marked for refinement 				unsigned int marked_cells = 0;
        //
        //				for (const auto &cell :
        //slab->space.tria->active_cell_iterators()) 					if
        //(cell->refine_flag_set()) 						marked_cells++;
        //
        //				DTM::pout << "\tSpace top fraction = " <<
        //std::setprecision(5) << ((double)marked_cells) /
        //slab->space.tria->n_active_cells() << std::endl;
        //
...
template <int dim>
void Fluid<dim>::eta_init_data_output() {
  Assert(parameter_set.use_count(), dealii::ExcNotInitialized());
  ...
  // may output data: initialise (mode: all, last or specific dwr loop)
  DTM::pout << "error indicators data output: patches = "
            << parameter_set->data_output.error_estimator.patches << std::endl;
  ...
  error_estimator.data_output_space->set_data_component_interpretation_field(
      dci_field);
  error_estimator.data_output_time->set_data_component_interpretation_field(
      dci_field);
  ...
  // check if we use a fixed trigger interval, or, do output once on a I_n
  if (!parameter_set->data_output.error_estimator.trigger_type.compare(
          "fixed")) {
  ...
  error_estimator.data_output_trigger =
      parameter_set->data_output.error_estimator.trigger;

  if (error_estimator.data_output_trigger_type_fixed) {
    DTM::pout
        << "error indicators data output: using fixed mode with trigger = "
        << error_estimator.data_output_trigger << std::endl;
  } else {
    DTM::pout << "error indicators data output: using I_n mode (trigger adapts "
                 "to I_n automatically)"
              << std::endl;
  }
...
template <int dim>
void Fluid<dim>::eta_do_data_output_on_slab(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator &eta,
    const unsigned int dwr_loop, std::string eta_type) {
  Assert(slab->space.pu.fe_info->dof.use_count(), dealii::ExcNotInitialized());
  Assert(eta->x[0].use_count(), dealii::ExcNotInitialized());
  // TODO: might need to be debugged; adapted from
  // primal_do_data_output_on_slab() & copied form dual_do_data_output_on_slab()
  //
  //	// triggered output mode
  //	Assert(slab->space.pu.fe_info->dof.use_count(),
  //dealii::ExcNotInitialized()); 	Assert(
  ...
  //	);
  //
  // 	auto eta_trigger = std::make_shared<
  // dealii::TrilinosWrappers::MPI::Vector > (); 	eta_trigger->reinit(
  // 		slab->space.pu.fe_info->dof->n_dofs()
  // 	);
  //
  std::ostringstream filename;
  filename << eta_type << "-dwr_loop-" << std::setw(setw_value_dwr_loops)
           << std::setfill('0') << dwr_loop;
  //
  //	{
  //		// fe face values time: time face (I_n) information
  ...
  //				error_estimator.data_output_time_value +=
  //error_estimator.data_output_trigger;
  //			}
  //
  //			if (output_times.size() && output_times.back() > t_n) {
  ...
  //
  //			std::vector< dealii::types::global_dof_index >
  //local_dof_indices(slab->time.pu.fe_info->fe->dofs_per_cell);
  //			cell_time->get_dof_indices(local_dof_indices);
  //
  //			for (unsigned int qt{0}; qt <
  //fe_values_time.n_quadrature_points; ++qt) {
  ...
  // { 				for ( 					dealii::types::global_dof_index i{0}; 					i <
  // slab->space.pu.fe_info->dof->n_dofs(); ++i) {
...
template <int dim>
void Fluid<dim>::eta_do_data_output_on_slab_Qn_mode(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator
        &eta_space,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator
  ...
  error_estimator.data_output_space->set_DoF_data(slab->space.pu.fe_info->dof);

  error_estimator.data_output_time->set_DoF_data(slab->space.pu.fe_info->dof);

  auto eta_trigger = std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  eta_trigger->reinit(*slab->space.pu.fe_info->locally_owned_dofs,
                      *slab->space.pu.fe_info->locally_relevant_dofs, mpi_comm);

  auto owned_tmp_space =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  owned_tmp_space->reinit(*slab->space.pu.fe_info->locally_owned_dofs,
                          mpi_comm);

  auto owned_tmp_time =
      std::make_shared<dealii::TrilinosWrappers::MPI::Vector>();
  owned_tmp_time->reinit(*slab->space.pu.fe_info->locally_owned_dofs, mpi_comm);

  std::ostringstream filename_space;
  filename_space << "error-indicators-space-dwr_loop-"
                 << std::setw(setw_value_dwr_loops) << std::setfill('0')
                 << dwr_loop;

  std::ostringstream filename_time;
  filename_time << "error-indicators-time-dwr_loop-"
                << std::setw(setw_value_dwr_loops) << std::setfill('0')
                << dwr_loop;
  {
    // create fe values
    dealii::FEValues<1> fe_values_time(
        *slab->time.pu.fe_info->mapping, *slab->time.pu.fe_info->fe,
        dealii::QGauss<1>(slab->time.pu.fe_info->fe->tensor_degree() +
                          1),  // PU = dG(0) in time -> QGauss(1)
        dealii::update_values | dealii::update_quadrature_points);
    ...
    for (; cell_time != endc_time; ++cell_time) {
      ...
      for (unsigned int qt{0}; qt < fe_values_time.n_quadrature_points; ++qt) {
        ...
        *eta_trigger = *owned_tmp_space;
        ...
        error_estimator.data_output_space->write_data(
            filename_space.str(), eta_trigger,
            // error_estimator.data_postprocessor,
            fe_values_time.quadrature_point(qt)[0]  // t_trigger
        );
...
template <int dim>
void Fluid<dim>::eta_do_data_output(
    const typename fluid::types::spacetime::dwr::slabs<dim>::iterator &slab,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator
        &eta_space,
    const typename DTM::types::storage_data_trilinos_vectors<1>::iterator
  ...
  if (error_estimator.data_output_dwr_loop < 0) return;

  if (static_cast<unsigned int>(error_estimator.data_output_dwr_loop) !=
      dwr_loop)
    return;
  ...
  if (!error_estimator.data_output_trigger_type_fixed) {
    ...
    // fixed trigger output mode
    eta_do_data_output_on_slab(slab, eta_space, dwr_loop,
                               "error-indicators-space");
    eta_do_data_output_on_slab(slab, eta_time, dwr_loop,
                               "error-indicators-time");
  }
}

template <int dim>
void Fluid<dim>::eta_sort_xdmf_by_time(const unsigned int dwr_loop) {
  if (dealii::Utilities::MPI::this_mpi_process(mpi_comm) == 0) {
    // name of xdmf file to be sorted by time of the snapshots
    std::ostringstream filename_space;
    filename_space << "error-indicators-space-dwr_loop-"
    ...
    for (auto filename_str : {filename_space.str(), filename_time.str()}) {
      ...
      if (!old_xdmf_file
               .good())  // exit this function, if there is no xdmf file
        return;
