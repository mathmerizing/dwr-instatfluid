...
FluidRHSAssembly<dim>::FluidRHSAssembly(
    ...
    : vi_rhs_vector(
      ...
      local_dof_indices(n_global_active_cells_t,  // n_cells_time
...
void Assembler<dim>::assemble(
  ...
  dealii::WorkStream::run(
      ...
      Assembly::CopyData::FluidRHSAssembly<dim>(*space.fe, *time.fe,
      ...
      for (unsigned int q{0}; q < scratch.space_fe_values.n_quadrature_points;
        ...
        for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii)
          for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i) {
            ...
            double u_i_ii =
                (*u)[scratch.space_local_dof_indices[i]
                     // time offset
                     + space.dof->n_dofs() * (n * time.fe->dofs_per_cell)
                     // local in time dof
                     + space.dof->n_dofs() * ii];
            // partial_t v(qt,qx)
            scratch.partial_t_v +=
        ...
        if (nonlin) {
          for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii)
            for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i) {
              copydata.vi_rhs_vector[n](i + ii * space.fe->dofs_per_cell) +=
                  // convection convection term
                  (scratch.space_phi[i] *
                   scratch.time_fe_values.shape_value(ii, qt) * scratch.grad_v *
                   scratch.v * scratch.spacetime_JxW);
            }
        }
            ...
            copydata.vi_rhs_vector[n](i + ii * space.fe->dofs_per_cell) +=
                ...
                + ((symmetric_stress
                        ? (scratch.space_symgrad_phi[i] *
                           ...
                           scratch.viscosity * 2. * scratch.symgrad_v)
                        : (scalar_product(
                ...
                - (scratch.space_div_phi[i] *
                   scratch.time_fe_values.shape_value(ii, qt) * scratch.p *
                   scratch.spacetime_JxW)
                // div-free constraint
                + (scratch.space_psi[i] *
                   scratch.time_fe_values.shape_value(ii, qt) * scratch.div_v *
                   scratch.spacetime_JxW);
          }
      }  // space qp
      ...
      for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii)
        for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i) {
          copydata.vi_rhs_vector[n](i + ii * space.fe->dofs_per_cell) +=
              ...
              scratch.space_fe_values[convection].value(i, q) *
              scratch.time_fe_face_values.shape_value(ii, 0) * scratch.u_plus *
        ...
        for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii)
          for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i) {
            ...
            double u_i_ii =
                (*u)[scratch.space_local_dof_indices[i]
                     // time offset
                     + space.dof->n_dofs() * ((n - 1) * time.fe->dofs_per_cell)
                     // local in time dof
                     + space.dof->n_dofs() * ii];
            // function eval u^-
            scratch.u_minus +=
...
void Assembler<dim>::copy_local_to_global_cell(
  ...
  for (unsigned int n{0}; n < copydata.vi_rhs_vector.size(); ++n) {
    spacetime.constraints->distribute_local_to_global(
        copydata.vi_rhs_vector[n], copydata.local_dof_indices[n], *Fu);
  }
}
