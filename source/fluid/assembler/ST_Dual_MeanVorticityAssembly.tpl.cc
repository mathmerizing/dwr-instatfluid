...
Je_MeanVorticityAssembly<dim>::Je_MeanVorticityAssembly(
    ...
    :  // init space
      ...
      primal_time_fe_values(
          primal_mapping_time, primal_fe_time, quad_time,
...
template <int dim>
Je_MeanVorticityAssembly<dim>::Je_MeanVorticityAssembly(
    const Je_MeanVorticityAssembly &scratch)
    :
...
Je_MeanVorticityAssembly<dim>::Je_MeanVorticityAssembly(
    ...
    : vi_Jei_vector(
          ...
          dealii::Vector<double>(fe_s.dofs_per_cell * fe_t.dofs_per_cell)),
...
template <int dim>
Je_MeanVorticityAssembly<dim>::Je_MeanVorticityAssembly(
    const Je_MeanVorticityAssembly &copydata)
...
void Assembler<dim>::assemble(
    ...
    std::shared_ptr<dealii::TrilinosWrappers::MPI::Vector> u, const double &t0,
    const double &T) {
  ////////////////////////////////////////////////////////////////////////////
  // check
  ...
  dealii::WorkStream::run(
      ...
      Assembly::Scratch::Je_MeanVorticityAssembly<dim>(
          ...
          *space.fe, *space.mapping, quad_space, *time.fe, *time.mapping,
          quad_time, *primal.space.fe, *primal.space.mapping, *primal.time.fe,
          *primal.time.mapping, t0, T),
      Assembly::CopyData::Je_MeanVorticityAssembly<dim>(
          *space.fe, *time.fe, time.n_global_active_cells));
  ...
  for (; cell_time != endc_time; ++cell_time, ++cell_primal) {
    ...
    for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i)
      for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii) {
        copydata.local_dof_indices[n][i + ii * space.fe->dofs_per_cell] =
      ...
      for (unsigned int q{0}; q < scratch.space_fe_values.n_quadrature_points;
        ...
        for (unsigned int ii{0}; ii < primal.time.fe->dofs_per_cell; ++ii)
          for (unsigned int i{0}; i < primal.space.fe->dofs_per_cell; ++i) {
            ...
            double u_i_ii = (*_u)[scratch.primal_space_local_dof_indices[i]
                                  // time offset
            ...
            // grad v
            scratch.primal_space_grad_v +=
                u_i_ii * scratch.primal_space_grad_phi[i];
          }

        scratch.curl_u = (scratch.primal_space_grad_v[1][0]) -
                         (scratch.primal_space_grad_v[0][1]);
        for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii)
          for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i) {
            scratch.curl_phi = (scratch.space_grad_phi[i][1][0] -
                                scratch.space_grad_phi[i][0][1]);

            copydata.vi_Jei_vector[n](i + ii * space.fe->dofs_per_cell) +=
                time_scaling * 2. * scratch.curl_u * scratch.curl_phi *
                scratch.space_fe_values.JxW(q) * scratch.time_fe_values.JxW(qt);
          }
      }
    }  // t_q
  }
...
void Assembler<dim>::copy_local_to_global_cell(
  ...
  Assert((copydata.vi_Jei_vector.size() == copydata.local_dof_indices.size()),
         dealii::ExcNotInitialized());

  for (unsigned int n{0}; n < copydata.vi_Jei_vector.size(); ++n) {
    spacetime.constraints->distribute_local_to_global(
        copydata.vi_Jei_vector[n], copydata.local_dof_indices[n], *_Je);
  }
}
