...
template<int dim>
DivFreeProjectionRHSAssembly<dim>::DivFreeProjectionRHSAssembly(const DivFreeProjectionRHSAssembly &scratch) :
	space_fe_values(
...
DivFreeProjectionRHSAssembly<dim>::DivFreeProjectionRHSAssembly(
	...
	vi_um_vector(fe_s.dofs_per_cell),
	local_dof_indices(fe_s.dofs_per_cell) {
}

template<int dim>
DivFreeProjectionRHSAssembly<dim>::DivFreeProjectionRHSAssembly(const DivFreeProjectionRHSAssembly &copydata) :
...
//	pressure = dim;
	...
	run(
		...
		Assembly::CopyData::DivFreeProjectionRHSAssembly<dim> (
			*space.fe
		)
	);
