...
Je_MeanDragAssembly<dim>::Je_MeanDragAssembly(
    ...
    :  // init space
       ...
       //	),
      space_fe_face_values(mapping_space, fe_space, face_quad_space,
...
template <int dim>
Je_MeanDragAssembly<dim>::Je_MeanDragAssembly(
    const Je_MeanDragAssembly &scratch)
    :  //	space_fe_values(
       ...
       //	),
      space_fe_face_values(scratch.space_fe_face_values.get_mapping(),
...
template <int dim>
Je_MeanDragAssembly<dim>::Je_MeanDragAssembly(
    const Je_MeanDragAssembly &copydata)
  ...
  *Je = 0.;
  ...
  dealii::WorkStream::run(
      ...
      Assembly::Scratch::Je_MeanDragAssembly<dim>(
          *space.fe, *space.mapping,
          //			quad_space,
          face_quad_space, *time.fe, *time.mapping, quad_time, t0, T),
      Assembly::CopyData::Je_MeanDragAssembly<dim>(*space.fe, *time.fe,
      ...
      for (unsigned int face = 0;
        ...
        if (cell->face(face)->at_boundary() &&
          ...
          for (unsigned int q{0};
               q < scratch.space_fe_face_values.n_quadrature_points; ++q) {
            ...
            for (unsigned int ii{0}; ii < time.fe->dofs_per_cell; ++ii)
              for (unsigned int i{0}; i < space.fe->dofs_per_cell; ++i) {
                ...
                if (symmetric_stress) {
                  sigma_fluid +=
                      scratch.viscosity * transpose(scratch.space_grad_v[i]);
                }
                const dealii::Tensor<1, dim> drag_lift_value =
                    -1.0 * sigma_fluid *
                    scratch.space_fe_face_values.normal_vector(q);

                copydata.vi_Jei_vector[n](i + ii * space.fe->dofs_per_cell) +=
                    time_scaling * 20. *  // valid for 2D-2 and 2D-3
                    (drag_lift_value[0] *
