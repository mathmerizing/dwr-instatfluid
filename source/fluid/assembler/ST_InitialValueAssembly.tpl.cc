...
template<int dim>
InitialValueAssembly<dim>::InitialValueAssembly(const InitialValueAssembly &scratch) :
	space_fe_values(
...
template<int dim>
InitialValueAssembly<dim>::InitialValueAssembly(const InitialValueAssembly &copydata) :
...
//	pressure = dim;
	...
	run(
		...
		Assembly::Scratch::InitialValueAssembly<dim> (
			*space.fe,
			*space.mapping,
			quad_space,
			*time.fe,
		...
		Assembly::CopyData::InitialValueAssembly<dim> (
			*space.fe,
			*time.fe
		)
	);
