...
template<int dim>
DivFreeProjectionAssembly<dim>::DivFreeProjectionAssembly( // @suppress("Class members should be properly initialized")
		// space
...
template<int dim>
DivFreeProjectionAssembly<dim>::DivFreeProjectionAssembly(const DivFreeProjectionAssembly &scratch) :
	space_fe_values(
...
template<int dim>
DivFreeProjectionAssembly<dim>::DivFreeProjectionAssembly(const DivFreeProjectionAssembly &copydata) :
...
//	Assert(function.viscosity.use_count(), dealii::ExcNotInitialized());
	...
	run(
		...
		Assembly::CopyData::DivFreeProjectionAssembly<dim> (
			*space.fe
		)
	);
