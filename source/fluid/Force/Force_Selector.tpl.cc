...
void Selector<dim>::create_function(
  ...
  DTM::pout << "* found configuration: fluid force function = " << _type
            << std::endl;
  ...
  if (_type.compare("ZeroTensorFunction") == 0) {
    AssertThrow(options.size() == 0,
                dealii::ExcMessage("fluid force options invalid, "
                                   "please check your input file data."));

    tensor_function =
        std::make_shared<dealii::ZeroTensorFunction<1, dim, double> >();

    DTM::pout << "fluid force selector: created zero tensor function"
              << std::endl
              << std::endl;
  ...
  if (_type.compare("ConstantTensorFunction") == 0) {
    ...
    DTM::pout << "fluid force selector: created ConstantTensorFunction "
  ...
  AssertThrow(
      false,
      dealii::ExcMessage(
          "fluid force function unknown, please check your input file data."));
}
