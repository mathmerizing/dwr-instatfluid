...
void Selector<dim>::create_function(
  ...
  DTM::pout << "* found configuration: viscosity function = " << _type
            << std::endl;
  ...
  if (_type.compare("ZeroFunction") == 0) {
    AssertThrow(options.size() == 0,
                dealii::ExcMessage("viscosity options invalid, "
                                   "please check your input file data."));

    function = std::make_shared<dealii::Functions::ZeroFunction<dim> >(1);

    DTM::pout << "viscosity selector: created zero function" << std::endl
              << std::endl;
  ...
  if (_type.compare("ConstantFunction") == 0) {
    ...
    function = std::make_shared<dealii::Functions::ConstantFunction<dim> >(
        std::stod(options.at(0)), 1);

    DTM::pout << "viscosity selector: created ConstantFunction "
  ...
  AssertThrow(
      false,
      dealii::ExcMessage(
          "viscosity function unknown, please check your input file data."));
}
