 ...
 * equipped with appropriate inital and boundary values.
 * with the partition of the boundary
 * \f$ \partial \Omega = \Gamma_D \cup \Gamma_N \f$
 * and \f$ \Gamma_D \neq \emptyset \f$.
 * 
 * 
...
//	AssertThrow(NumProc == 1, dealii::ExcMessage("MPI mode not supported."));
	...
	try {
		...
		AssertThrow(
			!(argc < 2),
			dealii::ExcMessage (
				std::string ("===>\tUSAGE: ./dwr-fluid <Input_Parameter_File.prm>"))
		);
		...
		auto parameter_handler =
			std::make_shared< fluid::ParameterHandler > ();
		...
		{
			DTM::pout
				<< "dwr-fluid solver: primal cG(p)-dG(r)"
				<< std::endl;
			
			switch (dimension) {
			case 2: {
				problem =
					std::make_shared< fluid::Fluid<2> > ();
				break;
			}
			
			case 3: {
				problem =
					std::make_shared< fluid::Fluid<3> > ();
				break;
			}
		...
		DTM::pout
			<< "dwr-fluid: dimension dim = " << dimension << std::endl
			<< std::endl;
