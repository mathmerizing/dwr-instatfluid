...
dealii::IndexSet extract_locally_owned_dofs(
    std::shared_ptr<dealii::DoFHandler<dim, spacedim>> space_dof,
    std::shared_ptr<dealii::DoFHandler<1, 1>> time_dof) {
  dealii::IndexSet space_owned_dofs = space_dof->locally_owned_dofs();

  dealii::IndexSet spacetime_owned_dofs(space_owned_dofs.size() *
                                        time_dof->n_dofs());

  for (dealii::types::global_dof_index time_dof_index{0};
       time_dof_index < time_dof->n_dofs(); time_dof_index++) {
...
dealii::IndexSet extract_locally_relevant_dofs(
  ...
  dealii::DoFTools::extract_locally_relevant_dofs(*space_dof,
                                                  space_relevant_dofs);
...
template <typename T>
void make_spacetime_constraints(
  ...
  for (auto id = space_relevant_dofs->begin(); id != space_relevant_dofs->end();
       id++) {
    ...
    if (space_constraints->is_constrained(*id)) {
      ...
      for (dealii::types::global_dof_index d{0}; d < n_time_dofs; ++d) {
        ...
        if (entries->size() > 0) {
          for (auto entry : *entries) {
            spacetime_constraints->add_entry(*id + d * n_space_dofs,
